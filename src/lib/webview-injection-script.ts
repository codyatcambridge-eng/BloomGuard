...
export interface NonShortsTransitionClearHoldInput {
  isMainSurfaceUrl: boolean;
  regularPathQuarantinePassed: boolean;
  isHomeShortsShelfVideo: boolean;
  isTransitionChurnReason: boolean;
  strictContinuityItemKey: string;
  reattachItemKey: string;
...
export function shouldHoldNonShortsTransitionClear(input: NonShortsTransitionClearHoldInput): boolean {
  if (!input || typeof input !== 'object') return false;
  if (!input.isMainSurfaceUrl) return false;
  if (!input.regularPathQuarantinePassed) return false;
  if (input.isHomeShortsShelfVideo) return false;
  if (!input.isTransitionChurnReason) return false;
  if (input.hasContextData) return false;
  if (!input.hasAnyBlur) return false;
  if (!input.classificationLocked) return false;
  if (!input.hasOwnershipProof) return false;
  const strictKey = String(input.strictContinuityItemKey || 'unknown');
  const reattachKey = String(input.reattachItemKey || 'unknown');
  const strictUnknown = !strictKey || strictKey === 'unknown';
  const reattachUnknown = !reattachKey || reattachKey === 'unknown';
  return strictUnknown && reattachUnknown;
}
...
export function generateModerationScript(config: InjectionConfig): string {
  ...
  mwDiagLog('[Netflix1] startup_diag_marker');
  mwDiagLog('[sunny1] startup_diag_marker');
  ...
  const state = {
    ...
    stats: {
      ...
      positiveTransitionBlurDroppedUnresolved: 0,
      positiveTransitionBlurDroppedWithContext: 0,
      overlayWithoutBlurRemoved: 0,
      duplicateOverlayRemoved: 0,
      overlayProvenanceVeto: 0,
      transitionStaleEpochSkipped: 0,
    },
    ...
    regularMainCardNegativeLatch: new Map(), // key -> { cardNodeId, navId, pageEpoch, itemKey, src, setAt, lastSeenAt, expiresAt, reason }
  };
  ...
  const diagTransitionMaxEpochByNav = new Map();
  ...
  const REGULAR_MAIN_CARD_NEGATIVE_LATCH_TTL_MS = 2600;
  const REGULAR_MAIN_CARD_NEGATIVE_LATCH_MAX = 220;
  ...
  function shouldHoldNonShortsTransitionClearLocal(input) {
    if (!input || typeof input !== 'object') return false;
    if (!input.isMainSurfaceUrl) return false;
    if (!input.regularPathQuarantinePassed) return false;
    if (input.isHomeShortsShelfVideo) return false;
    ...
    if (!input.classificationLocked) return false;
    if (!input.hasOwnershipProof) return false;
    var strictKey = String(input.strictContinuityItemKey || 'unknown');
    var reattachKey = String(input.reattachItemKey || 'unknown');
    var strictUnknown = !strictKey || strictKey === 'unknown';
    var reattachUnknown = !reattachKey || reattachKey === 'unknown';
    return strictUnknown && reattachUnknown;
  }
  ...
  function findHomeShortsShelfCardBoundaryNode(node) {
    if (!node || node.nodeType !== 1 || typeof node.closest !== 'function') return null;
    const shortsScopedCard = node.closest(
      'ytm-shorts-lockup-view-model-v2, ytm-shorts-lockup-view-model, ytd-reel-item-renderer, ytd-reel-video-renderer, ytm-grid-shelf-view-model, grid-shelf-view-model, ytm-rich-item-renderer, ytd-rich-item-renderer'
    );
    if (shortsScopedCard) return shortsScopedCard;
    return findRegularMainCardFallbackNode(node);
  }
  ...
  function pruneRegularMainCardNegativeLatch(now) {
    const latchMap = state.regularMainCardNegativeLatch;
    if (!latchMap || typeof latchMap.forEach !== 'function') return;
    latchMap.forEach(function(entry, key) {
      const expiresAt = Number(entry && entry.expiresAt ? entry.expiresAt : 0);
      if (!Number.isFinite(expiresAt) || expiresAt <= now) {
    ...
    if (latchMap.size <= REGULAR_MAIN_CARD_NEGATIVE_LATCH_MAX) return;
    const entries = Array.from(latchMap.entries());
    entries.sort(function(a, b) {
      const aAt = Number(a && a[1] && a[1].lastSeenAt ? a[1].lastSeenAt : 0);
      const bAt = Number(b && b[1] && b[1].lastSeenAt ? b[1].lastSeenAt : 0);
    ...
    const removeCount = Math.max(0, entries.length - REGULAR_MAIN_CARD_NEGATIVE_LATCH_MAX);
    for (let i = 0; i < removeCount; i += 1) {
      latchMap.delete(entries[i][0]);
    }
  }

  function writeRegularMainCardNegativeLatch(cardNodeId, itemKey, src, reason) {
    if (!cardNodeId || cardNodeId === 'none') return;
    const now = Date.now();
    pruneRegularMainCardNegativeLatch(now);
    ...
    const entry = {
      ...
      setAt: now,
      lastSeenAt: now,
      expiresAt: now + REGULAR_MAIN_CARD_NEGATIVE_LATCH_TTL_MS,
      reason: String(reason || 'unknown'),
    };
    state.regularMainCardNegativeLatch.set(key, entry);
  }

  function readRegularMainCardNegativeLatch(cardNodeId, itemKey, src) {
    if (!cardNodeId || cardNodeId === 'none') {
      return { matched: false, reason: 'missing_card_identity', entry: null };
    }
    const now = Date.now();
    pruneRegularMainCardNegativeLatch(now);
    const key = buildRegularMainCardBlurLatchKey(NAV_ID, state.pageEpoch, cardNodeId);
    const entry = state.regularMainCardNegativeLatch.get(key);
    if (!entry) return { matched: false, reason: 'no_entry_for_scope', entry: null };
    const entryExpiresAt = Number(entry.expiresAt || 0);
    if (!Number.isFinite(entryExpiresAt) || entryExpiresAt <= now) {
      state.regularMainCardNegativeLatch.delete(key);
      return { matched: false, reason: 'expired', entry: null };
    }
    const normalizedSrc = normalizeUrl(String(src || '')) || '';
    const normalizedEntrySrc = normalizeUrl(String(entry.src || '')) || '';
    const normalizedItemKey = String(itemKey || 'unknown');
    const normalizedEntryItemKey = String(entry.itemKey || 'unknown');
    const itemKeyMatch = !!(
      normalizedItemKey &&
      normalizedItemKey !== 'unknown' &&
      normalizedEntryItemKey &&
      normalizedEntryItemKey !== 'unknown' &&
      normalizedItemKey === normalizedEntryItemKey
    );
    const srcMatch = !!(
      normalizedSrc &&
      normalizedEntrySrc &&
      normalizedSrc === normalizedEntrySrc
    );
    if (!itemKeyMatch && !srcMatch) {
      return { matched: false, reason: 'identity_mismatch', entry: entry };
    }
    entry.lastSeenAt = now;
    entry.expiresAt = now + REGULAR_MAIN_CARD_NEGATIVE_LATCH_TTL_MS;
    state.regularMainCardNegativeLatch.set(key, entry);
    return { matched: true, reason: itemKeyMatch ? 'item_key_match' : 'src_match', entry: entry };
  }
  ...
  function stampRegularMainAuthoritativePositiveProof(node, src, reason) {
    ...
    node.dataset.mwRegularPositiveProof = '1';
    node.dataset.mwRegularPositiveProofNodeId = String(getDiagNodeId(node) || 'none');
    node.dataset.mwRegularPositiveProofCardNodeId = String(cardNodeId || 'none');
    node.dataset.mwRegularPositiveProofItemKey = String(itemKey || 'unknown');
    node.dataset.mwRegularPositiveProofSrc = String(normalizedSrc || '');
  ...
  function stampShortsShelfAuthoritativePositiveProof(node, src, strictKey, reason) {
    if (!node || node.nodeType !== 1 || !node.dataset) return;
    if (!isYouTubeMainPageThumbnailSurfaceUrl(window.location.href)) return;
    const shortsCardNode = (
      typeof node.closest === 'function'
        ? node.closest('ytm-shorts-lockup-view-model-v2, ytm-shorts-lockup-view-model, ytd-reel-item-renderer, ytd-reel-video-renderer, ytm-rich-item-renderer, ytd-rich-item-renderer')
        : null
    );
    const cardNodeId = shortsCardNode ? getDiagNodeId(shortsCardNode) : 'none';
    const normalizedSrc = normalizeUrl(String(src || (node.dataset && node.dataset.mwSrc) || '')) || '';
    const srcItemKey = getDiagItemKey(normalizedSrc || String(src || (node.dataset && node.dataset.mwSrc) || ''));
    const proofItemKey = (
      strictKey && strictKey !== 'unknown'
        ? String(strictKey)
        : String(srcItemKey || 'unknown')
    );
    node.dataset.mwShortsShelfPositiveProof = '1';
    node.dataset.mwShortsShelfPositiveProofNodeId = String(getDiagNodeId(node) || 'none');
    node.dataset.mwShortsShelfPositiveProofCardNodeId = String(cardNodeId || 'none');
    node.dataset.mwShortsShelfPositiveProofItemKey = String(proofItemKey || 'unknown');
    node.dataset.mwShortsShelfPositiveProofSrc = String(normalizedSrc || '');
    node.dataset.mwShortsShelfPositiveProofNav = String(NAV_ID || 'none');
    node.dataset.mwShortsShelfPositiveProofPageEpoch = String(state.pageEpoch || 0);
    node.dataset.mwShortsShelfPositiveProofAt = String(Date.now());
    node.dataset.mwKnownNegativeForCardOrItem = '0';
    if (reason) {
      node.dataset.mwShortsShelfPositiveProofReason = String(reason || 'unknown');
    }
  }

  function stampRegularMainNegativeProof(node, src, reason) {
    if (!node || node.nodeType !== 1 || !node.dataset) return;
    if (isShortsModeActive()) return;
    if (!isYouTubeMainPageThumbnailSurfaceUrl(window.location.href)) return;
    const cardNode = findNonShortsStrongCardNode(node) || findRegularMainCardFallbackNode(node);
    ...
    const normalizedSrc = normalizeUrl(String(src || (node.dataset && node.dataset.mwSrc) || '')) || '';
    const itemKey = getDiagItemKey(normalizedSrc || String(src || (node.dataset && node.dataset.mwSrc) || ''));
    node.dataset.mwKnownNegativeForCardOrItem = '1';
    node.dataset.mwRegularNegativeProof = '1';
    node.dataset.mwRegularNegativeProofCardNodeId = String(cardNodeId || 'none');
    node.dataset.mwRegularNegativeProofItemKey = String(itemKey || 'unknown');
    node.dataset.mwRegularNegativeProofSrc = String(normalizedSrc || '');
  ...
  function isYouTubeMainPageShortsShelfVideoNode(node) {
    ...
    // [Patch34] Detect Shorts shelf promoted player promoted outside ytm-shorts-lockup-view-model
    // into a grid-shelf-view-model container. YouTube mobile homepage promotes the shared VIDEO
    // player to the grid-shelf level, so closest('ytm-shorts-lockup-view-model') returns null.
    const gridShelf = node.closest('ytm-grid-shelf-view-model, grid-shelf-view-model');
    if (gridShelf && typeof gridShelf.querySelector === 'function') {
      if (gridShelf.querySelector('ytm-shorts-lockup-view-model')) return true;
    }
  ...
  // [Patch35] Probe the grid-shelf for the currently-active ytm-shorts-lockup-view-model.
  // YouTube marks the active item in the Shorts shelf carousel with one of several
  // state attributes/classes. We try them in priority order and fall back to the
  // DOM-first anchor so behaviour is never worse than Patch34.
  function findActiveGridShelfShortsAnchor(gridShelfEl, nodeId) {
    if (!gridShelfEl || typeof gridShelfEl.querySelector !== 'function') return null;

    var ACTIVE_SELECTORS = [
      'ytm-shorts-lockup-view-model[active] a[href*="/shorts/"]',
      'ytm-shorts-lockup-view-model[focused] a[href*="/shorts/"]',
      'ytm-shorts-lockup-view-model[is-active] a[href*="/shorts/"]',
      'ytm-shorts-lockup-view-model[selected] a[href*="/shorts/"]',
      'ytm-shorts-lockup-view-model.active a[href*="/shorts/"]',
      'ytm-shorts-lockup-view-model.focused a[href*="/shorts/"]',
      'ytm-shorts-lockup-view-model.ytm-shorts-lockup-view-model--selected a[href*="/shorts/"]',
    ];

    for (var si = 0; si < ACTIVE_SELECTORS.length; si += 1) {
      try {
        var hit = gridShelfEl.querySelector(ACTIVE_SELECTORS[si]);
        if (hit) {
          console.log(
            '[DIAG][HOME_SHORTS_KEY_DERIVE][Patch35] active_lockup_selector_hit',
            'nodeId=' + (nodeId || 'unknown'),
            'selector=' + ACTIVE_SELECTORS[si]
          );
          return hit;
        }
      } catch (e) {}
    ...
    // No active-state selector fired — log attributes of the first lockup so we
    // can see in Xcode logs what YouTube actually stamps on the active item.
    try {
      var firstLockup = gridShelfEl.querySelector('ytm-shorts-lockup-view-model');
      if (firstLockup) {
        var attrDump = [];
        var attrs = firstLockup.attributes || [];
        for (var ai = 0; ai < Math.min(attrs.length, 12); ai += 1) {
          attrDump.push(attrs[ai].name + '=' + String(attrs[ai].value || '').substring(0, 40));
        }
        console.log(
          '[DIAG][HOME_SHORTS_KEY_DERIVE][Patch35] active_selector_miss_lockup_attrs',
          'nodeId=' + (nodeId || 'unknown'),
          'attrs=' + attrDump.join(' | ')
        );
      }
    ...
    // Fall back to DOM-first anchor (Patch34 behaviour).
    return gridShelfEl.querySelector('a[href*="/shorts/"]') || null;
  }
  ...
  function rememberNonShortsReattachContext(node, src, category, itemId, blurPx, reason) {
    ...
    if (homeShortsAnchorId === 'unknown') {
      homeShortsAnchorId = deriveNearbyShortsAnchorId(node);
    }
    if (homeShortsAnchorId === 'unknown' && typeof node.closest === 'function') {
      const gridShelfEl = node.closest('ytm-grid-shelf-view-model, grid-shelf-view-model');
      if (gridShelfEl) {
        const gridAnchor = findActiveGridShelfShortsAnchor(gridShelfEl, getDiagNodeId(node));
        if (gridAnchor) {
          const gridHref = String(
            (gridAnchor.getAttribute && gridAnchor.getAttribute('href')) ||
            gridAnchor.href ||
            ''
          );
          const gridId = extractShortsIdFromHrefValue(gridHref);
          if (gridId && gridId !== 'unknown') {
            homeShortsAnchorId = String(gridId);
          }
        }
      }
    }
    ...
    if (cardKey === 'none' && homeShortsAnchorId !== 'unknown') {
      console.log(
        '[DIAG][HOME_SHORTS_KEY_BIND] remembered_without_card',
        'shortsId=' + homeShortsAnchorId,
        'itemKey=' + itemKey,
        'cardNodeId=none',
        'reason=' + (reason || 'unknown')
  ...
  function diagNonShortsReattach(videoNode, reason, transitionEpoch) {
    if (!isNonShortsYouTubeReattachContext()) return;
    if (!videoNode || videoNode.nodeType !== 1) return;
    if (String(videoNode.tagName || '').toUpperCase() !== 'VIDEO') return;
    ...
    const navEpochKey = String(NAV_ID || 'none');
    const transitionEpochNumber = Number(transitionEpoch);
    const currentTransitionEpoch = (
      Number.isFinite(transitionEpochNumber) && transitionEpochNumber > 0
    ...
    if (
      ...
      Number.isFinite(nodeMaxEpoch) &&
      nodeMaxEpoch > 0 &&
      currentTransitionEpoch < nodeMaxEpoch
    ) {
      incrementMvpTransitionCounter(
        'transitionStaleEpochSkipped',
        'reason=' + String(reason || 'unknown') +
        ' navId=' + navEpochKey +
        ' staleEpoch=' + String(currentTransitionEpoch) +
        ' nodeMaxEpoch=' + String(nodeMaxEpoch) +
    ...
    if (
      Number.isFinite(currentTransitionEpoch) &&
      currentTransitionEpoch > 0 &&
      (!Number.isFinite(nodeMaxEpoch) || currentTransitionEpoch > nodeMaxEpoch) &&
      videoNode &&
      videoNode.dataset
    ) {
      videoNode.dataset.mwTransitionMaxEpoch = String(currentTransitionEpoch);
    }
    if (Number.isFinite(currentTransitionEpoch) && currentTransitionEpoch > 0) {
      const previousMaxEpoch = Number(diagTransitionMaxEpochByNav.get(navEpochKey) || 0);
      if (previousMaxEpoch > 0 && currentTransitionEpoch < previousMaxEpoch) {
        incrementMvpTransitionCounter(
          'transitionStaleEpochSkipped',
          'reason=' + String(reason || 'unknown') +
          ' navId=' + navEpochKey +
          ' staleEpoch=' + String(currentTransitionEpoch) +
          ' maxEpoch=' + String(previousMaxEpoch) +
          ' nodeId=' + String(getDiagNodeId(videoNode) || 'none')
        );
        return;
      }
      if (currentTransitionEpoch > previousMaxEpoch) {
        diagTransitionMaxEpochByNav.set(navEpochKey, currentTransitionEpoch);
      }
    }
    ...
    postNonShortsTransitionDiag('hit', {
      ...
      transitionEpoch: Number.isFinite(currentTransitionEpoch) ? currentTransitionEpoch : 0,
    });
    const now = Date.now();
    ...
    let regularUnresolvedNoProofForcedClear = false;
    let regularUnresolvedContextTerminalVetoActive = false;
    ...
    if (isHomeShortsShelfVideo && String(cardNodeId || 'none') === 'none') {
      const shortsBoundaryCard = findHomeShortsShelfCardBoundaryNode(videoNode);
      if (shortsBoundaryCard) {
        card = shortsBoundaryCard;
        cardNodeId = getDiagNodeId(shortsBoundaryCard);
        postNonShortsTransitionDiag('shorts_card_boundary_fallback_adopted', {
          reason: String(reason || 'unknown'),
          nodeId: videoNodeId,
          marker: 'MW-FLICKER-SHORTS-CARD-BOUNDARY-FALLBACK-V1',
          cardNodeId: String(cardNodeId || 'none'),
        });
      }
    }
    ...
    // never fires, leaving any incidental blur unprotected.
    if (
      isHomeShortsShelfVideo &&
      strictContinuityItemKey === 'unknown' &&
      typeof videoNode.closest === 'function'
    ) {
      const gridShelfEl = videoNode.closest('ytm-grid-shelf-view-model, grid-shelf-view-model');
      if (gridShelfEl && typeof gridShelfEl.querySelector === 'function') {
        const gridAnchor = findActiveGridShelfShortsAnchor(gridShelfEl, videoNodeId);
        if (gridAnchor) {
          const gridHref = String(
            (gridAnchor.getAttribute && gridAnchor.getAttribute('href')) ||
            gridAnchor.href ||
          ...
          const gridM = gridHref.match(/\\/shorts\\/([^/?#]+)/);
          if (gridM && gridM[1]) {
            derivedAnchorItemKey = String(gridM[1]);
            strictContinuityItemKey = derivedAnchorItemKey;
            console.log(
              '[DIAG][HOME_SHORTS_KEY_DERIVE] grid_shelf_fallback',
              'nodeId=' + videoNodeId,
              'derivedKey=' + derivedAnchorItemKey,
              'reason=' + (reason || 'unknown'),
              'patch=35'
    ...
    let laneFlipRegularToShortsNoCardIdentity = false;
    if (isMainSurfaceUrl) {
      ...
      if (
        ...
        laneFlipRegularToShortsNoCardIdentity = !!(
          previousLaneState.lane === 'regular' &&
          nextLane === 'shorts' &&
    ...
    if (isMainSurfaceUrl && isHomeShortsShelfVideo) {
      const shortsCardBoundaryCandidate = (
        card ||
        findHomeShortsShelfCardBoundaryNode(videoNode) ||
    ...
    if (isMainSurfaceUrl && !contextData && (normalizedPoster || normalizedCurrent)) {
      const regularMainSrcMatchAllowed = !!(
        regularPathQuarantinePassed &&
        !isHomeShortsShelfVideo &&
        String(cardNodeId || 'none') !== 'none'
      );
      ...
      for (let i = 0; i < blurredCandidates.length; i += 1) {
        ...
        if (!regularMainSrcMatchAllowed) continue;
        const candidateCard = findNonShortsStrongCardNode(candidate) || findRegularMainCardFallbackNode(candidate);
        const candidateCardNodeId = candidateCard ? getDiagNodeId(candidateCard) : 'none';
        if (candidateCardNodeId === 'none' || candidateCardNodeId !== String(cardNodeId || 'none')) {
          continue;
        }
        if (
    ...
    if (isHomeShortsShelfVideo && hasAuthoritativeBlur) {
      ...
      stampShortsShelfAuthoritativePositiveProof(
        videoNode,
        String((videoNode.dataset && videoNode.dataset.mwSrc) || normalizedPoster || normalizedCurrent || ''),
        String(strictContinuityItemKey || 'unknown'),
        'shorts_shelf_authoritative'
      );
    }
    ...
    const earlyRegularProofNav = String((videoNode.dataset && videoNode.dataset.mwRegularPositiveProofNav) || 'none');
    const earlyRegularProofPageEpoch = Number((videoNode.dataset && videoNode.dataset.mwRegularPositiveProofPageEpoch) || 0);
    const earlyRegularProofNodeId = String((videoNode.dataset && videoNode.dataset.mwRegularPositiveProofNodeId) || 'none');
    const earlyRegularProofCardNodeId = String((videoNode.dataset && videoNode.dataset.mwRegularPositiveProofCardNodeId) || 'none');
    const earlyRegularProofSrc = normalizeUrl(String((videoNode.dataset && videoNode.dataset.mwRegularPositiveProofSrc) || '')) || '';
    const earlyPreviousAuthoritativePositive = String((videoNode.dataset && videoNode.dataset.mwRegularPositiveProof) || '') === '1';
    const earlySameNavEpoch = !!(
      earlyPreviousAuthoritativePositive &&
      earlyRegularProofNav === String(NAV_ID || 'none') &&
      earlyRegularProofPageEpoch > 0 &&
      earlyRegularProofPageEpoch === Number(state.pageEpoch || 0)
    );
    const earlySameDomNode = !!(
      earlyRegularProofNodeId &&
      earlyRegularProofNodeId !== 'none' &&
      earlyRegularProofNodeId === String(videoNodeId || 'none')
    );
    const earlyReplacementNodeSameCard = !!(
      !earlySameDomNode &&
      earlyRegularProofCardNodeId &&
      earlyRegularProofCardNodeId !== 'none' &&
      String(cardNodeId || 'none') !== 'none' &&
      earlyRegularProofCardNodeId === String(cardNodeId || 'none')
    );
    const earlyCurrentResolvedSrcForProof = normalizeUrl(String(normalizedPoster || normalizedCurrent || (videoNode.dataset && videoNode.dataset.mwSrc) || '')) || '';
    const earlyStrongPositiveContinuityForUnresolvedClear = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      isTransitionChurnReason &&
      hasAuthoritativeBlur &&
      ...
      earlyPreviousAuthoritativePositive &&
      earlySameNavEpoch &&
      (earlySameDomNode || earlyReplacementNodeSameCard) &&
      !!earlyRegularProofSrc &&
      !!earlyCurrentResolvedSrcForProof &&
      earlyRegularProofSrc === earlyCurrentResolvedSrcForProof &&
      !!hardBlurSrc &&
      hardBlurSrc === earlyCurrentResolvedSrcForProof &&
      !(
        String(cardNodeId || 'none') === 'none' &&
        reattachItemKey === 'unknown' &&
        (!strictIdentityKnown || strictContinuityItemKey === 'unknown')
    ...
    const unresolvedHoldOwnershipProof = !!(
      (
        strictIdentityKnown &&
        hardItemKeyKnown &&
        strictContinuityItemKey === hardBlurItemKey
    ...
    const classificationLockedForTransition = !!isNodeClassificationLocked(videoNode, reason);
    const unresolvedClearHoldByProof = shouldHoldNonShortsTransitionClearLocal({
      isMainSurfaceUrl: !!isMainSurfaceUrl,
      regularPathQuarantinePassed: !!regularPathQuarantinePassed,
      isHomeShortsShelfVideo: !!isHomeShortsShelfVideo,
      isTransitionChurnReason: !!isTransitionChurnReason,
      strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
      reattachItemKey: String(reattachItemKey || 'unknown'),
      hasContextData: !!contextData,
      hasAnyBlur: !!(hasAuthoritativeBlur || hasIncidentalBlur),
      classificationLocked: !!classificationLockedForTransition,
      hasOwnershipProof: !!unresolvedHoldOwnershipProof,
    });
    const unresolvedClearHoldUntil = Number(
      (videoNode && videoNode.dataset && videoNode.dataset.mwNonShortsUnresolvedHoldUntil) || 0
    );
    const unresolvedClearHoldActive = !!(
      Number.isFinite(unresolvedClearHoldUntil) &&
      unresolvedClearHoldUntil > Date.now()
    );
    if (unresolvedClearHoldByProof) {
      if (videoNode && videoNode.dataset) {
        videoNode.dataset.mwNonShortsUnresolvedHoldUntil =
          String(Date.now() + NON_SHORTS_UNRESOLVED_CLEAR_HOLD_MS);
        videoNode.dataset.mwNonShortsUnresolvedNoProofSince = '0';
      }
      holdBlurDuringUnresolvedTransition = true;
      postNonShortsTransitionDiag('regular_main_unresolved_clear_blocked', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-UNRESOLVED-HOLD-V1',
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        classificationLocked: !!classificationLockedForTransition,
      });
    } else if (unresolvedClearHoldActive && unresolvedHoldOwnershipProof) {
      holdBlurDuringUnresolvedTransition = true;
      postNonShortsTransitionDiag('regular_main_unresolved_clear_hold_ttl', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-UNRESOLVED-HOLD-V1',
        holdUntil: Number(unresolvedClearHoldUntil || 0),
      });
    } else if (videoNode && videoNode.dataset) {
      if (Number(videoNode.dataset.mwNonShortsUnresolvedHoldUntil || 0) > 0) {
        videoNode.dataset.mwNonShortsUnresolvedHoldUntil = '0';
      }
      if (!contextData && !unresolvedHoldOwnershipProof) {
        const unresolvedNoProofSince = Number(videoNode.dataset.mwNonShortsUnresolvedNoProofSince || 0);
        if (!Number.isFinite(unresolvedNoProofSince) || unresolvedNoProofSince <= 0) {
          videoNode.dataset.mwNonShortsUnresolvedNoProofSince = String(Date.now());
        } else if (
          Date.now() - unresolvedNoProofSince >= NON_SHORTS_UNRESOLVED_CLEAR_HOLD_MS &&
          (hasAuthoritativeBlur || hasIncidentalBlur) &&
          !earlyStrongPositiveContinuityForUnresolvedClear
        ) {
          const clearedNoProof = clearAllBlurAndOverlay(
            videoNode,
            normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
            'regular_unresolved_no_proof_decontaminate',
            'safe'
          );
          if (clearedNoProof) {
            ...
            regularUnresolvedNoProofForcedClear = true;
    ...
    const shortsShelfGlobalContextBlockedOnLaneFlip = !!(
      isMainSurfaceUrl &&
      isHomeShortsShelfVideo &&
      !!contextData &&
      String(cardNodeId || 'none') === 'none' &&
    ...
    if (shortsShelfGlobalContextBlockedOnLaneFlip) {
      console.log(
        '[MW-MVP-SHORTS-SHELF-GLOBAL-CONTEXT-BLOCK-V1] context_blocked_lane_flip_no_card_identity',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
      ...
      postNonShortsTransitionDiag('shorts_shelf_global_context_blocked_lane_flip_no_card_identity', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-SHORTS-SHELF-GLOBAL-CONTEXT-BLOCK-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        nearbyShortsAnchorId: String(nearbyShortsAnchorId || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    const shortsShelfGlobalContextFailClosedUnresolved = !!(
      isMainSurfaceUrl &&
      isHomeShortsShelfVideo &&
      !!contextData &&
      String(cardNodeId || 'none') === 'none' &&
      ...
      (!hardItemKeyKnown || String(hardBlurItemKey || 'unknown') === 'unknown')
    );
    if (shortsShelfGlobalContextFailClosedUnresolved) {
      console.log(
        '[MW-MVP-SHORTS-SHELF-GLOBAL-CONTEXT-FAIL-CLOSED-V1] context_blocked_unresolved_global_without_hard_proof',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
      ...
      postNonShortsTransitionDiag('shorts_shelf_global_context_fail_closed_unresolved', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-SHORTS-SHELF-GLOBAL-CONTEXT-FAIL-CLOSED-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    if (
      ...
      (
        ...
        (regularNoCardBoundaryUnknownIdentity && !unresolvedHoldOwnershipProof)
      ) &&
      ...
      postNonShortsTransitionDiag('hard_blur_provenance_mismatch_cleared', {
        ...
        postNonShortsTransitionDiag('regular_main_negative_decontaminate_skipped_strong_positive_continuity', {
          reason: String(reason || 'unknown'),
          nodeId: videoNodeId,
          marker: 'MW-MVP-REGULAR-NEGATIVE-DECONTAMINATE-SKIPPED-POSITIVE-CONTINUITY-V1',
          previousAuthoritativePositive: !!earlyPreviousAuthoritativePositive,
          sameDomNode: !!earlySameDomNode,
          replacementNodeSameCard: !!earlyReplacementNodeSameCard,
          sameNavEpoch: !!earlySameNavEpoch,
          proofSrcMatchesCurrent: !!(
            earlyRegularProofSrc &&
            earlyCurrentResolvedSrcForProof &&
            earlyRegularProofSrc === earlyCurrentResolvedSrcForProof
          ),
    ...
    const regularProofNodeId = String((videoNode.dataset && videoNode.dataset.mwRegularPositiveProofNodeId) || 'none');
    ...
    const sameNavEpoch = !!(
      ...
      regularProofNav === String(NAV_ID || 'none') &&
      regularProofPageEpoch > 0 &&
      regularProofPageEpoch === Number(state.pageEpoch || 0)
    );
    const sameDomNode = !!(
      regularProofNodeId &&
      regularProofNodeId !== 'none' &&
      regularProofNodeId === String(videoNodeId || 'none')
    );
    const replacementNodeSameCard = !!(
      !sameDomNode &&
      regularProofCardNodeId &&
      regularProofCardNodeId !== 'none' &&
      String(cardNodeId || 'none') !== 'none' &&
      regularProofCardNodeId === String(cardNodeId || 'none')
    );
    const cardBoundaryChanged = !!(
      regularProofCardNodeId &&
      regularProofCardNodeId !== 'none' &&
      String(cardNodeId || 'none') !== 'none' &&
      regularProofCardNodeId !== String(cardNodeId || 'none')
    );
    const currentResolvedSrcForProof = normalizeUrl(String(normalizedPoster || normalizedCurrent || (videoNode.dataset && videoNode.dataset.mwSrc) || '')) || '';
    const transitionItemKeyForNegativeLatch = (
      (strictIdentityKnown && strictContinuityItemKey && strictContinuityItemKey !== 'unknown' && String(strictContinuityItemKey)) ||
      (reattachItemKey && reattachItemKey !== 'unknown' && String(reattachItemKey)) ||
      getDiagItemKey(currentResolvedSrcForProof || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''))
    );
    const knownNegativeLocalNode = String((videoNode.dataset && videoNode.dataset.mwKnownNegativeForCardOrItem) || '') === '1';
    const regularMainNegativeLatchRead = readRegularMainCardNegativeLatch(
      String(cardNodeId || 'none'),
      String(transitionItemKeyForNegativeLatch || 'unknown'),
      String(currentResolvedSrcForProof || '')
    );
    const knownNegativeForCardOrItem = !!(knownNegativeLocalNode || regularMainNegativeLatchRead.matched);
    const priorHardBlurProofSameNode = !!(previousAuthoritativePositive && sameNavEpoch && sameDomNode);
    const priorHardBlurProofSameCard = !!(previousAuthoritativePositive && sameNavEpoch && replacementNodeSameCard);
    const priorHardBlurProofSameItemKey = !!(
      priorHardBlurProofSameNode &&
      regularProofItemKey &&
      regularProofItemKey !== 'unknown' &&
      hardItemKeyKnown &&
      regularProofItemKey === String(hardBlurItemKey || 'unknown')
    );
    const priorHardBlurProofSameReattachKey = !!(
      priorHardBlurProofSameNode &&
      regularProofItemKey &&
      regularProofItemKey !== 'unknown' &&
      reattachItemKey &&
      reattachItemKey !== 'unknown' &&
      regularProofItemKey === String(reattachItemKey || 'unknown')
    );
    const priorHardBlurProofSameLocalSrc = !!(
      priorHardBlurProofSameNode &&
      regularProofSrc &&
    ...
    const priorHardBlurProofSameCardItemKey = !!(
    ...
    const priorHardBlurProofSameCardReattachKey = !!(
      priorHardBlurProofSameCard &&
      regularProofItemKey &&
      regularProofItemKey !== 'unknown' &&
      reattachItemKey &&
    ...
    const priorHardBlurProofSameCardLocalSrc = !!(
      priorHardBlurProofSameCard &&
      regularProofSrc &&
      currentResolvedSrcForProof &&
      regularProofSrc === currentResolvedSrcForProof &&
    ...
    const unresolvedRegularTransitionNow = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      isMainSurfaceUrl &&
      isTransitionChurnReason &&
    ...
    const regularStrongOwnershipForUnresolvedHold = !!(
      (
        String(cardNodeId || 'none') !== 'none' &&
        strictIdentityKnown &&
        hardItemKeyKnown &&
      ...
      (
        ...
        reattachItemKey !== 'unknown' &&
        hardItemKeyKnown &&
        String(reattachItemKey || 'unknown') === String(hardBlurItemKey || 'unknown')
      ) ||
      (
        !!contextData &&
        (
          String(contextKind || 'none') === 'card_blurred' ||
          String(contextKind || 'none') === 'itemkey_card' ||
          String(contextKind || 'none') === 'regular_main_preclear_card_recovery'
        ) &&
    ...
    const regularPreVetoContinuityProof = !!(
      unresolvedRegularTransitionNow &&
      (!regularNoCardBoundaryUnknownIdentity || regularStrongOwnershipForUnresolvedHold) &&
      previousAuthoritativePositive &&
      sameNavEpoch &&
      sameDomNode &&
      !cardBoundaryChanged &&
      ...
      (
        ...
        (priorHardBlurProofSameLocalSrc && !regularSameLocalSrcCarryBlockedUnknownIdentity) ||
        (
          regularProofSrc &&
          currentResolvedSrcForProof &&
          regularProofSrc === currentResolvedSrcForProof
    ...
    const regularUnresolvedWeakCarryBlocked = !!(
      unresolvedRegularTransitionNow &&
      regularNoCardBoundaryUnknownIdentity &&
      !regularStrongOwnershipForUnresolvedHold &&
      !regularPreVetoContinuityProof
    );
    const regularPositivePriorProof = !!(
      !regularUnresolvedWeakCarryBlocked &&
      !knownNegativeForCardOrItem &&
      !cardBoundaryChanged &&
      (
        (
    ...
    const regularPositiveHoldSafetyDenied = !!(
      confirmedLiveGhostByItemKey ||
      regularUnknownIdentityContextBlock ||
      regularContextFailClosedUnproven ||
      laneFlipShortsToRegularUnknownIdentity ||
      regularLaneFlipGhostQuarantineActive ||
      regularGhostEpochBypassActive
    );
    const regularPositiveHoldEligible = !!(
      unresolvedRegularTransitionNow &&
    ...
    const regularPositiveHoldEffectiveEligible = !!(
      regularPositiveHoldEligible ||
      unresolvedPositiveHoldPrecedenceStrong
    );
    const wouldClearWithoutHold = !!(!contextData && !(activeVideoBlurred || hasAuthoritativeBlur));
    let regularPositiveHoldProofKind = 'none';
    if (priorHardBlurProofSameItemKey) regularPositiveHoldProofKind = 'same_item_key';
    else if (priorHardBlurProofSameReattachKey) regularPositiveHoldProofKind = 'same_reattach_key';
    else if (priorHardBlurProofSameLocalSrc && !regularSameLocalSrcCarryBlockedUnknownIdentity) regularPositiveHoldProofKind = 'same_local_src';
    else if (priorHardBlurProofSameCardItemKey) regularPositiveHoldProofKind = 'replacement_same_card_item_key';
    else if (priorHardBlurProofSameCardReattachKey) regularPositiveHoldProofKind = 'replacement_same_card_reattach_key';
    else if (priorHardBlurProofSameCardLocalSrc) regularPositiveHoldProofKind = 'replacement_same_card_local_src';
    else if (
      priorHardBlurProofSameNode &&
      !regularUnresolvedWeakCarryBlocked &&
      !regularNoCardBoundaryUnknownIdentity
    ) regularPositiveHoldProofKind = 'same_node_authoritative';
    else if (priorHardBlurProofSameCard) regularPositiveHoldProofKind = 'replacement_same_card';
    if (
      regularPositiveHoldProofKind === 'none' &&
      unresolvedPositiveHoldPrecedenceStrong &&
      !regularUnresolvedWeakCarryBlocked
    ) {
      regularPositiveHoldProofKind = 'same_node_src_precedence';
    }
    const holdNow = Date.now();
    let regularPositiveHoldStartedAt = Number((videoNode.dataset && videoNode.dataset.mwRegularPositiveHoldStartedAt) || 0);
    let regularPositiveHoldUntil = Number((videoNode.dataset && videoNode.dataset.mwRegularPositiveHoldUntil) || 0);
    let regularPositiveHoldTtlRemaining = 0;
    let regularClearVetoStartedAt = Number((videoNode.dataset && videoNode.dataset.mwRegularClearVetoStartedAt) || 0);
    let regularClearVetoUntil = Number((videoNode.dataset && videoNode.dataset.mwRegularClearVetoUntil) || 0);
    let regularClearVetoTtlRemaining = 0;
    const regularClearVetoEligible = !!(
      unresolvedRegularTransitionNow &&
      !regularUnresolvedWeakCarryBlocked &&
    ...
    if (regularUnresolvedWeakCarryBlocked) {
      postNonShortsTransitionDiag('regular_unresolved_weak_carry_veto', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-UNRESOLVED-WEAK-CARRY-VETO-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        hardSrcMatchesCurrentExact: !!hardSrcMatchesCurrentExact,
        hasStrongOwnershipProof: !!regularStrongOwnershipForUnresolvedHold,
        action: 'clear_blur',
      });
    }
    if (regularPositiveHoldEffectiveEligible) {
      if (!Number.isFinite(regularPositiveHoldStartedAt) || regularPositiveHoldStartedAt <= 0) {
        regularPositiveHoldStartedAt = holdNow;
      }
      const holdElapsed = holdNow - regularPositiveHoldStartedAt;
      if (holdElapsed <= MAX_REGULAR_POSITIVE_HOLD_TOTAL_MS) {
        const cappedUntil = regularPositiveHoldStartedAt + MAX_REGULAR_POSITIVE_HOLD_TOTAL_MS;
        regularPositiveHoldUntil = Math.min(holdNow + NON_SHORTS_UNRESOLVED_CLEAR_HOLD_MS, cappedUntil);
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwRegularPositiveHoldStartedAt = String(regularPositiveHoldStartedAt);
          videoNode.dataset.mwRegularPositiveHoldUntil = String(regularPositiveHoldUntil);
        }
      } else {
        regularPositiveHoldUntil = 0;
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwRegularPositiveHoldStartedAt = '0';
          videoNode.dataset.mwRegularPositiveHoldUntil = '0';
        }
      }
    } else if (contextData && videoNode && videoNode.dataset) {
      videoNode.dataset.mwRegularPositiveHoldStartedAt = '0';
      videoNode.dataset.mwRegularPositiveHoldUntil = '0';
    }
    if (regularClearVetoEligible) {
      if (!Number.isFinite(regularClearVetoStartedAt) || regularClearVetoStartedAt <= 0) {
        regularClearVetoStartedAt = holdNow;
      }
      const vetoElapsed = holdNow - regularClearVetoStartedAt;
      if (vetoElapsed <= MAX_REGULAR_POSITIVE_HOLD_TOTAL_MS) {
        const vetoCappedUntil = regularClearVetoStartedAt + MAX_REGULAR_POSITIVE_HOLD_TOTAL_MS;
        regularClearVetoUntil = Math.min(holdNow + NON_SHORTS_UNRESOLVED_CLEAR_VETO_MS, vetoCappedUntil);
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwRegularClearVetoStartedAt = String(regularClearVetoStartedAt);
          videoNode.dataset.mwRegularClearVetoUntil = String(regularClearVetoUntil);
        }
      } else {
        regularClearVetoUntil = 0;
        if (videoNode && videoNode.dataset) {
    ...
    regularPositiveHoldTtlRemaining = Math.max(0, regularPositiveHoldUntil - holdNow);
    regularClearVetoTtlRemaining = Math.max(0, regularClearVetoUntil - holdNow);
    const regularClearVetoKeepBlur = !!(regularClearVetoEligible && regularClearVetoTtlRemaining > 0);
    const regularPositiveHoldKeepBlur = !!(
      (regularPositiveHoldEffectiveEligible && regularPositiveHoldTtlRemaining > 0) ||
      regularClearVetoKeepBlur
    );
    const hasAnyNegativeProof = !!(
      knownNegativeForCardOrItem ||
      regularMainNegativeLatchRead.matched ||
      confirmedLiveGhostByItemKey
    );
    const hasPositiveContinuityProofStrong = !!(
      unresolvedRegularTransitionNow &&
      (!regularNoCardBoundaryUnknownIdentity || regularStrongOwnershipForUnresolvedHold) &&
      previousAuthoritativePositive &&
      !cardBoundaryChanged &&
      ...
      (
        ...
        (sameDomNode && sameNavEpoch) ||
        priorHardBlurProofSameItemKey ||
        priorHardBlurProofSameReattachKey ||
        (priorHardBlurProofSameLocalSrc && !regularSameLocalSrcCarryBlockedUnknownIdentity)
      )
    );
    if (priorHardBlurProofSameLocalSrc && regularSameLocalSrcCarryBlockedUnknownIdentity) {
      postNonShortsTransitionDiag('regular_same_local_src_carry_blocked_unknown_identity', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-SAME-LOCAL-SRC-CARRY-BLOCKED-UNKNOWN-IDENTITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
    ...
    const regularTerminalForcedClearBlockedByContinuity = !!(
      regularTerminalKeepBlurByContinuity &&
      !hasAnyNegativeProof
    );
    const regularKeepBlurStrongOwnershipProof = !!(
      (
        String(cardNodeId || 'none') !== 'none' &&
        strictIdentityKnown &&
        hardItemKeyKnown &&
    ...
    const regularUnresolvedNoCardWeakSameItemQuarantine = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      unresolvedRegularTransitionNow &&
      isTransitionChurnReason &&
      ...
      !hardSrcMatchesCurrentExact &&
      !regularKeepBlurStrongOwnershipProof &&
      !regularTerminalKeepBlurByContinuity
    );
    const regularUnresolvedNoCardNoStrongOwnershipVeto = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      unresolvedRegularTransitionNow &&
      isTransitionChurnReason &&
    ...
    const regularPositiveContinuitySignalForUnresolved = !!(
      regularPositiveContinuitySignal &&
      (
        !regularNoCardBoundaryUnknownIdentity ||
        regularKeepBlurStrongOwnershipProof
    ...
    if (regularUnresolvedNoCardWeakSameItemQuarantine) {
      postNonShortsTransitionDiag('regular_unresolved_quarantine_enter', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-NEGATIVE-QUARANTINE-BLOCK-SAMEITEM-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        hardSrcMatchesCurrentExact: !!hardSrcMatchesCurrentExact,
        priorHardBlurProofSameItemKey: !!priorHardBlurProofSameItemKey,
        action: 'quarantine_enter',
      });
    }
    if (regularUnresolvedNoCardNoStrongOwnershipVeto) {
      postNonShortsTransitionDiag('regular_unresolved_no_strong_ownership_veto_enter', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-NEGATIVE-UNRESOLVED-OWNERSHIP-VETO-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        ...
        priorHardBlurProofSameItemKey: !!priorHardBlurProofSameItemKey,
        priorHardBlurProofSameReattachKey: !!priorHardBlurProofSameReattachKey,
        priorHardBlurProofSameLocalSrc: !!priorHardBlurProofSameLocalSrc,
        action: 'quarantine_enter',
      });
    }
    ...
    const regularLegacyUnresolvedKeepBlurGate = !!(
      unresolvedRegularTransitionNow &&
      !regularClearAllowedByProof
    );
    const regularUnresolvedKeepBlurGate = !!(
      regularLegacyUnresolvedKeepBlurGate &&
      hasPositiveContinuityProofStrong
    );
    let regularShouldKeepBlur = !!(
      regularPositiveHoldKeepBlur ||
      regularUnresolvedKeepBlurGate
    );
    if (regularUnresolvedNoProofForcedClear && !regularTerminalForcedClearBlockedByContinuity) {
      regularShouldKeepBlur = false;
      postNonShortsTransitionDiag('regular_unresolved_no_proof_terminal_clear_veto_keep_blur', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-UNRESOLVED-TERMINAL-CLEAR-V1',
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        contextFound: !!contextData,
    ...
    } else if (regularUnresolvedNoProofForcedClear && regularTerminalForcedClearBlockedByContinuity) {
      regularUnresolvedNoProofForcedClear = false;
      postNonShortsTransitionDiag('regular_terminal_forced_clear_blocked_positive_continuity', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-TERMINAL-FORCED-CLEAR-BLOCKED-POSITIVE-CONTINUITY-V1',
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
    ...
    if (regularUnresolvedNoCardWeakSameItemQuarantine) {
      regularShouldKeepBlur = false;
      postNonShortsTransitionDiag('regular_keep_blur_veto_unresolved_quarantine', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-NEGATIVE-QUARANTINE-BLOCK-SAMEITEM-V1',
        cardNodeId: String(cardNodeId || 'none'),
      ...
      if (hasAuthoritativeBlur || hasIncidentalBlur || activeVideoBlurred) {
        const quarantineCleared = clearAllBlurAndOverlay(
          videoNode,
          normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
          'regular_unresolved_quarantine_veto_same_item_key',
          'safe'
        );
    ...
    if (regularUnresolvedNoCardNoStrongOwnershipVeto) {
      regularShouldKeepBlur = false;
      postNonShortsTransitionDiag('regular_keep_blur_veto_unresolved_no_strong_ownership', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-NEGATIVE-UNRESOLVED-OWNERSHIP-VETO-V1',
        cardNodeId: String(cardNodeId || 'none'),
      ...
      if (hasAuthoritativeBlur || hasIncidentalBlur || activeVideoBlurred) {
        const ownershipVetoCleared = clearAllBlurAndOverlay(
          videoNode,
          normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
          'regular_unresolved_no_strong_ownership_veto_clear',
          'safe'
        );
        if (ownershipVetoCleared) {
          hasAuthoritativeBlur = false;
          hasIncidentalBlur = false;
          activeVideoBlurred = false;
          holdBlurDuringUnresolvedTransition = false;
          if (!regularTerminalForcedClearBlockedByContinuity) {
            regularUnresolvedNoProofForcedClear = true;
          } else {
            postNonShortsTransitionDiag('regular_terminal_forced_clear_blocked_positive_continuity', {
              reason: String(reason || 'unknown'),
              ...
              sameNavEpoch: !!sameNavEpoch,
              action: 'keep_blur',
              source: 'regular_unresolved_no_strong_ownership_veto_clear',
            });
          }
        }
      }
    ...
    if (regularPositiveHoldProofKind === 'none' && regularUnresolvedKeepBlurGate && hasPositiveContinuityProofStrong) {
      regularPositiveHoldProofKind = 'continuity_fail_closed';
    }
    if (unresolvedRegularTransitionNow) {
      if (regularPositiveHoldEffectiveEligible) {
        console.log(
          '[MW-MVP-REGULAR-POSITIVE-HOLD-ELIGIBLE-V1]',
          'nodeId=' + String(videoNodeId || 'none'),
          'cardNodeId=' + String(cardNodeId || 'none'),
          'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
          'hardBlurItemKey=' + String(hardBlurItemKey || 'unknown'),
          ...
          'cardBoundaryChanged=' + String(!!cardBoundaryChanged),
          'knownNegativeForCardOrItem=' + String(!!knownNegativeForCardOrItem),
          'negativeLatchMatch=' + String(!!regularMainNegativeLatchRead.matched),
          'negativeLatchReason=' + String(regularMainNegativeLatchRead.reason || 'none'),
          'contextFound=' + String(!!contextData),
          'priorProof=' + String(!!regularPositivePriorProof),
          'wouldClearWithoutHold=' + String(!!wouldClearWithoutHold),
          'ttlRemaining=' + String(regularPositiveHoldTtlRemaining),
          'action=' + String(regularPositiveHoldKeepBlur ? 'keep_blur' : 'clear_blur'),
          'srcLineage=' + String((regularProofSrc || currentResolvedSrcForProof || '')).substring(0, 180)
        );
      } else {
        console.log(
          '[MW-MVP-REGULAR-POSITIVE-HOLD-DENIED-V1]',
          'nodeId=' + String(videoNodeId || 'none'),
          'cardNodeId=' + String(cardNodeId || 'none'),
          'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
          'hardBlurItemKey=' + String(hardBlurItemKey || 'unknown'),
          ...
          'wouldClearWithoutHold=' + String(!!wouldClearWithoutHold),
          'ttlRemaining=' + String(regularPositiveHoldTtlRemaining),
          'action=' + String(regularClearAllowedByProof ? 'clear_blur' : 'keep_blur_fail_closed'),
          'srcLineage=' + String((regularProofSrc || currentResolvedSrcForProof || '')).substring(0, 180)
        );
      }
    }
    if (
      ...
      !regularClearAllowedByProof &&
      hasPositiveContinuityProofStrong &&
      !regularUnresolvedNoCardWeakSameItemQuarantine
    ) {
      postNonShortsTransitionDiag('regular_clear_blocked_positive_continuity', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-CLEAR-BLOCKED-POSITIVE-CONTINUITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    } else if (regularLegacyUnresolvedKeepBlurGate && !regularUnresolvedKeepBlurGate) {
      postNonShortsTransitionDiag('regular_unresolved_keep_blur_veto_no_strong_proof', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-UNRESOLVED-KEEP-VETO-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    } else if (regularClearAllowedByProof) {
      postNonShortsTransitionDiag('regular_clear_allowed_negative_proof', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-CLEAR-ALLOWED-NEGATIVE-PROOF-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    if (regularShouldKeepBlur) {
      ...
      markAuthoritativeHardBlur(videoNode, String((videoNode.dataset && videoNode.dataset.mwSrc) || currentResolvedSrcForProof || ''));
      stampRegularMainAuthoritativePositiveProof(videoNode, String((videoNode.dataset && videoNode.dataset.mwSrc) || currentResolvedSrcForProof || ''), 'regular_positive_hold_keep_blur');
      hasAuthoritativeBlur = true;
      hasIncidentalBlur = true;
      activeVideoBlurred = true;
      postNonShortsTransitionDiag('regular_positive_hold_keep_blur', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-POSITIVE-HOLD-KEEP-BLUR-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        ...
        previousAuthoritativePositive: !!previousAuthoritativePositive,
        sameNavEpoch: !!sameNavEpoch,
        cardBoundaryChanged: !!cardBoundaryChanged,
        knownNegativeForCardOrItem: !!knownNegativeForCardOrItem,
        negativeLatchMatch: !!regularMainNegativeLatchRead.matched,
        negativeLatchReason: String(regularMainNegativeLatchRead.reason || 'none'),
        contextFound: !!contextData,
        priorProof: !!regularPositivePriorProof,
        wouldClearWithoutHold: !!wouldClearWithoutHold,
        ttlRemaining: Number(regularPositiveHoldTtlRemaining || 0),
        action: 'keep_blur',
        srcLineage: String((regularProofSrc || currentResolvedSrcForProof || '')).substring(0, 180),
      });
    } else if (regularClearAllowedByProof) {
      postNonShortsTransitionDiag('regular_positive_hold_clear_blur', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-POSITIVE-HOLD-CLEAR-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    const shortsProofNodeId = String((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProofNodeId) || 'none');
    const shortsProofCardNodeId = String((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProofCardNodeId) || 'none');
    const shortsProofItemKey = String((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProofItemKey) || 'unknown');
    const shortsProofSrc = normalizeUrl(String((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProofSrc) || '')) || '';
    const shortsProofNav = String((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProofNav) || 'none');
    const shortsProofPageEpoch = Number((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProofPageEpoch) || 0);
    const previousShortsAuthoritativePositive = String((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveProof) || '') === '1';
    const shortsSameNavEpoch = !!(
      previousShortsAuthoritativePositive &&
      shortsProofNav === String(NAV_ID || 'none') &&
      shortsProofPageEpoch > 0 &&
      shortsProofPageEpoch === Number(state.pageEpoch || 0)
    );
    const shortsSameDomNode = !!(
      shortsProofNodeId &&
      shortsProofNodeId !== 'none' &&
      shortsProofNodeId === String(videoNodeId || 'none')
    );
    const shortsReplacementNodeSameCard = !!(
      !shortsSameDomNode &&
      shortsProofCardNodeId &&
      shortsProofCardNodeId !== 'none' &&
      String(cardNodeId || 'none') !== 'none' &&
      shortsProofCardNodeId === String(cardNodeId || 'none')
    );
    const shortsCardBoundaryChanged = !!(
      shortsProofCardNodeId &&
      shortsProofCardNodeId !== 'none' &&
      String(cardNodeId || 'none') !== 'none' &&
      shortsProofCardNodeId !== String(cardNodeId || 'none')
    );
    const shortsCurrentResolvedSrcForProof = normalizeUrl(
      String(normalizedPoster || normalizedCurrent || (videoNode.dataset && videoNode.dataset.mwSrc) || '')
    ) || '';
    const shortsPriorProofSameItemKey = !!(
      shortsProofItemKey &&
      shortsProofItemKey !== 'unknown' &&
      strictContinuityItemKey &&
      strictContinuityItemKey !== 'unknown' &&
      shortsProofItemKey === String(strictContinuityItemKey || 'unknown')
    );
    const shortsPriorProofSameLocalSrc = !!(
      shortsProofSrc &&
    ...
    const shortsShelfPriorProof = !!(
      ...
      (
        shortsPriorProofSameItemKey ||
        shortsPriorProofSameLocalSrc
      )
    );
    let shortsShelfProofKind = 'none';
    if (shortsPriorProofSameItemKey) shortsShelfProofKind = 'same_item_key';
    ...
    let shortsShelfHoldStartedAt = Number((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveHoldStartedAt) || 0);
    let shortsShelfHoldUntil = Number((videoNode.dataset && videoNode.dataset.mwShortsShelfPositiveHoldUntil) || 0);
    let shortsShelfHoldTtlRemaining = 0;
    const shortsShelfHoldNow = Date.now();
    if (unresolvedShortsShelfTransitionNow && shortsShelfPriorProof) {
      if (!Number.isFinite(shortsShelfHoldStartedAt) || shortsShelfHoldStartedAt <= 0) {
        shortsShelfHoldStartedAt = shortsShelfHoldNow;
      }
      const holdElapsed = shortsShelfHoldNow - shortsShelfHoldStartedAt;
      if (holdElapsed <= MAX_SHORTS_SHELF_POSITIVE_HOLD_TOTAL_MS) {
        const cappedUntil = shortsShelfHoldStartedAt + MAX_SHORTS_SHELF_POSITIVE_HOLD_TOTAL_MS;
        shortsShelfHoldUntil = Math.min(shortsShelfHoldNow + SHORTS_SHELF_POSITIVE_HOLD_MS, cappedUntil);
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwShortsShelfPositiveHoldStartedAt = String(shortsShelfHoldStartedAt);
          videoNode.dataset.mwShortsShelfPositiveHoldUntil = String(shortsShelfHoldUntil);
        }
      } else {
        shortsShelfHoldUntil = 0;
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwShortsShelfPositiveHoldStartedAt = '0';
          videoNode.dataset.mwShortsShelfPositiveHoldUntil = '0';
        }
      }
    } else if (contextData && videoNode && videoNode.dataset) {
      videoNode.dataset.mwShortsShelfPositiveHoldStartedAt = '0';
    ...
    shortsShelfHoldTtlRemaining = Math.max(0, shortsShelfHoldUntil - shortsShelfHoldNow);
    const shortsShelfHoldKeepBlur = !!(
    ...
    if (unresolvedShortsShelfTransitionNow) {
      postNonShortsTransitionDiag(
        shortsShelfPriorProof
          ? 'shorts_shelf_positive_hold_eligible'
          : 'shorts_shelf_positive_hold_denied',
        {
          reason: String(reason || 'unknown'),
          nodeId: String(videoNodeId || 'none'),
          marker: shortsShelfPriorProof
            ? 'MW-MVP-SHORTS-SHELF-POSITIVE-HOLD-ELIGIBLE-V1'
            : 'MW-MVP-SHORTS-SHELF-POSITIVE-HOLD-DENIED-V1',
          cardNodeId: String(cardNodeId || 'none'),
          strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
          hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
          reattachItemKey: String(reattachItemKey || 'unknown'),
          proofKind: String(shortsShelfProofKind || 'none'),
          sameDomNode: !!shortsSameDomNode,
          replacementNodeSameCard: !!shortsReplacementNodeSameCard,
          previousAuthoritativePositive: !!previousShortsAuthoritativePositive,
          sameNavEpoch: !!shortsSameNavEpoch,
          cardBoundaryChanged: !!shortsCardBoundaryChanged,
          knownNegativeForCardOrItem: !!knownNegativeForCardOrItem,
          contextFound: !!contextData,
    ...
    if (shortsShelfHoldKeepBlur) {
      ...
      videoNode.dataset.mwModerated = 'blurred';
      videoNode.dataset.mwClassifiedEpoch = String(CONFIG.pageEpoch);
      markAuthoritativeHardBlur(videoNode, String((videoNode.dataset && videoNode.dataset.mwSrc) || shortsCurrentResolvedSrcForProof || ''));
      ...
      stampShortsShelfAuthoritativePositiveProof(
        videoNode,
        String((videoNode.dataset && videoNode.dataset.mwSrc) || shortsCurrentResolvedSrcForProof || ''),
        String(strictContinuityItemKey || 'unknown'),
        'shorts_shelf_positive_hold_keep_blur'
      );
      hasAuthoritativeBlur = true;
      hasIncidentalBlur = true;
      activeVideoBlurred = true;
      postNonShortsTransitionDiag('shorts_shelf_positive_hold_keep_blur', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-SHORTS-SHELF-POSITIVE-HOLD-KEEP-BLUR-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
      ...
      postNonShortsTransitionDiag('shorts_shelf_positive_hold_clear_blur', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-SHORTS-SHELF-POSITIVE-HOLD-CLEAR-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    const shortsNoContextLocalBlurPositiveProof = !!(
      previousShortsAuthoritativePositive &&
      shortsSameNavEpoch &&
      !knownNegativeForCardOrItem &&
      (
    ...
    const shortsNoContextLocalBlurCandidate = !!(
      isMainSurfaceUrl &&
      isHomeShortsShelfVideo &&
      !contextData &&
      strictContinuityItemKey &&
    ...
    const shortsNoContextLocalBlurHold = !!(
      shortsNoContextLocalBlurCandidate &&
      shortsNoContextLocalBlurPositiveProof
    );
    if (shortsNoContextLocalBlurCandidate && !shortsNoContextLocalBlurPositiveProof) {
      postNonShortsTransitionDiag('shorts_no_context_local_blur_promotion_veto', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-SHORTS-NOCTX-PROMOTION-VETO-NEGATIVE-SAFE-V1',
        action: 'clear_blur',
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        hasIncidentalBlur: !!hasIncidentalBlur,
        ...
        sameNavEpoch: !!shortsSameNavEpoch,
        knownNegativeForCardOrItem: !!knownNegativeForCardOrItem,
        proofSameItemKey: !!shortsPriorProofSameItemKey,
        proofSameLocalSrc: !!shortsPriorProofSameLocalSrc,
        proofKind: String(shortsShelfProofKind || 'none'),
      });
    }
    if (shortsNoContextLocalBlurHold) {
      ...
      !contextData &&
      !preserveShortsHardBlurDuringGraceWindow &&
      !regularPositiveContinuitySignalForUnresolved
    ) {
      if (isMainSurfaceUrl && !isHomeShortsShelfVideo && regularPathQuarantinePassed) {
        postNonShortsTransitionDiag('regular_provenance_clear_source', {
          reason: String(reason || 'unknown'),
    ...
    const unresolvedLaneFlipNoIdentityNow = !!(
      isMainSurfaceUrl &&
      isTransitionChurnReason &&
      (
        laneFlipShortsToRegularUnknownIdentity ||
    ...
    const currentNodeResolvedSrc = normalizeUrl(
      String(normalizedPoster || normalizedCurrent || (videoNode.dataset && videoNode.dataset.mwSrc) || '')
    ) || '';
    const currentNodeResolvedItemKey = getDiagItemKey(currentNodeResolvedSrc || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''));
    const contextSourceForAdoption = normalizeUrl(
      String((contextData && contextData.src) || (contextNode && contextNode.dataset && contextNode.dataset.mwSrc) || '')
    ) || '';
    const contextSourceItemKey = getDiagItemKey(contextSourceForAdoption || String((contextData && contextData.src) || ''));
    const contextMatchesCurrentNodeIdentity = !!(
      contextSourceForAdoption &&
      currentNodeResolvedSrc &&
      contextSourceForAdoption === currentNodeResolvedSrc
    );
    const contextMatchesResolvedItemKey = !!(
      contextSourceItemKey &&
      contextSourceItemKey !== 'unknown' &&
      (
        (reattachItemKey && reattachItemKey !== 'unknown' && contextSourceItemKey === String(reattachItemKey || 'unknown')) ||
        (strictContinuityItemKey && strictContinuityItemKey !== 'unknown' && contextSourceItemKey === String(strictContinuityItemKey || 'unknown')) ||
        (currentNodeResolvedItemKey && currentNodeResolvedItemKey !== 'unknown' && contextSourceItemKey === String(currentNodeResolvedItemKey || 'unknown'))
      )
    );
    const blockSrcOnlyContextOwnership = !!(
      isMainSurfaceUrl &&
      isTransitionChurnReason &&
      (
        (
    ...
    const contextOwnedByCurrentNode = !!(
      contextData &&
      (
        contextNode === videoNode ||
        (
          contextMatchesCurrentNodeIdentity &&
          !blockSrcOnlyContextOwnership
        ) ||
        contextMatchesResolvedItemKey
      )
    );
    if (blockSrcOnlyContextOwnership && contextMatchesCurrentNodeIdentity && !contextMatchesResolvedItemKey) {
      postNonShortsTransitionDiag('context_src_only_ownership_blocked', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-CONTEXT-SRC-ONLY-OWNERSHIP-BLOCKED-V1',
        contextKind: String(contextKind || 'none'),
        contextItemKey: String(contextSourceItemKey || 'unknown'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        currentNodeResolvedItemKey: String(currentNodeResolvedItemKey || 'unknown'),
        contextOwnedByCurrentNode: false,
        isHomeShortsShelfVideo: !!isHomeShortsShelfVideo,
      });
    }
    const unresolvedLaneFlipApplyBlurVeto = !!(
      ...
      !regularPositiveHoldEligible &&
      !shortsShelfHoldKeepBlur &&
      !regularPositiveContinuitySignalForUnresolved
    );
    if (unresolvedLaneFlipApplyBlurVeto) {
      console.log(
        '[MW-MVP-REGULAR-LANE-FLIP-APPLY-BLUR-VETO-V1] veto_apply_blur_unresolved_lane_flip_unknown_identity',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
      ...
      postNonShortsTransitionDiag('regular_main_lane_flip_apply_blur_veto', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-LANE-FLIP-APPLY-BLUR-VETO-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        contextItemKey: String(contextSourceItemKey || 'unknown'),
        laneFlipShortsToRegularUnknownIdentity: !!laneFlipShortsToRegularUnknownIdentity,
        laneFlipRegularToShortsNoCardIdentity: !!laneFlipRegularToShortsNoCardIdentity,
    ...
    const unresolvedContextAdoptionNoProof = !!(
      contextData &&
      isMainSurfaceUrl &&
      isTransitionChurnReason &&
      !contextOwnedByCurrentNode &&
    ...
    const regularUnresolvedContextApplyVeto = !!(
      contextData &&
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      unresolvedRegularTransitionNow &&
    ...
    if (
      ...
      postNonShortsTransitionDiag('regular_main_unresolved_context_veto_skipped_positive_precedence', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-POSITIVE-CONTINUITY-PRECEDENCE-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
    ...
    if (regularUnresolvedContextApplyVeto) {
      console.log(
        '[MW-MVP-REGULAR-UNRESOLVED-CONTEXT-VETO-TERMINAL-V1] veto_context_adoption_regular_unresolved_no_strong_proof',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
        'reattachItemKey=' + String(reattachItemKey || 'unknown'),
        'contextItemKey=' + String(contextSourceItemKey || 'unknown'),
        'contextOwnedByCurrentNode=' + String(!!contextOwnedByCurrentNode)
      );
      postNonShortsTransitionDiag('regular_main_unresolved_context_apply_veto', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-UNRESOLVED-CONTEXT-VETO-TERMINAL-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        contextItemKey: String(contextSourceItemKey || 'unknown'),
      ...
      contextKind = 'none';
      contextNode = null;
      regularUnresolvedContextTerminalVetoActive = true;
      if (hasAuthoritativeBlur || hasIncidentalBlur || activeVideoBlurred) {
        const contextVetoCleared = clearAllBlurAndOverlay(
          videoNode,
          normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
          'regular_unresolved_context_veto_terminal_clear',
          'safe'
        );
        if (contextVetoCleared) {
          hasAuthoritativeBlur = false;
          hasIncidentalBlur = false;
          activeVideoBlurred = false;
          holdBlurDuringUnresolvedTransition = false;
          if (!regularTerminalForcedClearBlockedByContinuity) {
            ...
            postNonShortsTransitionDiag('regular_terminal_forced_clear_blocked_positive_continuity', {
              ...
              sameNavEpoch: !!sameNavEpoch,
              action: 'keep_blur',
              source: 'regular_unresolved_context_veto_terminal_clear',
            });
          }
        }
      }
    }
    if (unresolvedContextAdoptionNoProof) {
      console.log(
        '[MW-MVP-REGULAR-UNRESOLVED-ADOPTION-VETO-V1] veto_context_adoption_unresolved_no_local_proof',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
      ...
      postNonShortsTransitionDiag('regular_main_unresolved_context_adoption_veto', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-UNRESOLVED-ADOPTION-VETO-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        contextItemKey: String(contextSourceItemKey || 'unknown'),
    ...
    const regularNearbyShortsCrossSurfaceContextVeto = !!(
      contextData &&
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      nearbyShortsGuardSkippedNonShorts &&
    ...
    if (regularNearbyShortsCrossSurfaceContextVeto) {
      console.log(
        '[MW-MVP-REGULAR-CROSS-SURFACE-CONTEXT-VETO-V1] veto_context_adoption_non_shorts_nearby_shorts_guard',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
      ...
      postNonShortsTransitionDiag('regular_main_cross_surface_context_veto', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-CROSS-SURFACE-CONTEXT-VETO-V1',
        contextKind: String(contextKind || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        contextItemKey: String(contextSourceItemKey || 'unknown'),
      ...
      regularUnresolvedContextTerminalVetoActive = true;
      if (hasAuthoritativeBlur || hasIncidentalBlur || activeVideoBlurred) {
        const crossSurfaceVetoCleared = clearAllBlurAndOverlay(
          videoNode,
          normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
          'regular_cross_surface_context_veto_terminal_clear',
          'safe'
        );
        if (crossSurfaceVetoCleared) {
          hasAuthoritativeBlur = false;
          hasIncidentalBlur = false;
          activeVideoBlurred = false;
          holdBlurDuringUnresolvedTransition = false;
          if (!regularTerminalForcedClearBlockedByContinuity) {
            ...
            postNonShortsTransitionDiag('regular_terminal_forced_clear_blocked_positive_continuity', {
              ...
              sameNavEpoch: !!sameNavEpoch,
              action: 'keep_blur',
              source: 'regular_cross_surface_context_veto_terminal_clear',
            });
          }
        }
      }
    }
    const regularPositiveContinuityRehydrateBlur = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      unresolvedRegularTransitionNow &&
      isTransitionChurnReason &&
      regularShouldKeepBlur &&
      !hasAnyNegativeProof &&
      !contextData &&
      !hasAuthoritativeBlur &&
      !hasIncidentalBlur &&
      ...
      (
        ...
        (
          hardBlurSrc &&
          currentResolvedSrcForProof &&
          hardBlurSrc === currentResolvedSrcForProof
        )
      )
    );
    if (regularPositiveContinuityRehydrateBlur) {
      const rehydrateSrc = String(
        currentResolvedSrcForProof ||
        regularProofSrc ||
        hardBlurSrc ||
        normalizedPoster ||
      ...
      const rehydrateCategory = String((videoNode.dataset && videoNode.dataset.mwCategory) || 'flagged');
      const rehydrateItemId = String((videoNode.dataset && videoNode.dataset.mwItemId) || '');
      const rehydrateBlurStrength = IS_YOUTUBE ? 40 : Math.min(CONFIG.blurStrength || 30, 20);
      if (rehydrateSrc) {
        applyBlur(videoNode, rehydrateSrc, rehydrateCategory, rehydrateBlurStrength, rehydrateItemId);
        markAuthoritativeHardBlur(videoNode, rehydrateSrc);
        stampRegularMainAuthoritativePositiveProof(videoNode, rehydrateSrc, 'regular_positive_continuity_rehydrate');
        hasAuthoritativeBlur = isAuthoritativeHardBlur(videoNode);
        activeVideoBlurred = hasAuthoritativeBlur || hasIncidentalBlur;
        postNonShortsTransitionDiag('regular_main_positive_continuity_rehydrate_blur', {
          reason: String(reason || 'unknown'),
          nodeId: videoNodeId,
          marker: 'MW-MVP-REGULAR-POSITIVE-CONTINUITY-REHYDRATE-BLUR-V1',
          sameDomNode: !!sameDomNode,
          replacementNodeSameCard: !!replacementNodeSameCard,
          sameNavEpoch: !!sameNavEpoch,
          previousAuthoritativePositive: !!previousAuthoritativePositive,
    ...
    const cardBlurredContextAdoptionUnproven = !!(
      contextData &&
      String(contextKind || 'none') === 'card_blurred' &&
      !contextOwnedByCurrentNode &&
      (
    ...
    if (activeVideoBlurred && hasAuthoritativeBlur) {
      ...
      if (!hasExpectedBlur) {
        ...
        stampRegularMainAuthoritativePositiveProof(videoNode, String((videoNode.dataset && videoNode.dataset.mwSrc) || ''), 'regular_reapply_blur');
        videoNode.classList.add('mw-blurred');
        console.log(
      ...
      contextData &&
      !cardBlurredContextAdoptionUnproven &&
      !(
        regularPathQuarantinePassed &&
        !isHomeShortsShelfVideo &&
        unresolvedRegularTransitionNow &&
    ...
    } else if (cardBlurredContextAdoptionUnproven) {
      postNonShortsTransitionDiag('context_card_blurred_adoption_fail_closed', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-CARD-BLURRED-ADOPTION-FAIL-CLOSED-V1',
        contextKind: String(contextKind || 'none'),
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        ...
        hasStrongOwnershipProof: !!regularKeepBlurStrongOwnershipProof,
        hasPositiveContinuityProofStrong: !!hasPositiveContinuityProofStrong,
        shortsShelfHoldKeepBlur: !!shortsShelfHoldKeepBlur,
        shortsNoContextLocalBlurHold: !!shortsNoContextLocalBlurHold,
        action: 'clear_blur',
        terminal: true,
      });
      const cleared = clearAllBlurAndOverlay(
        videoNode,
        normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
        'context_card_blurred_adoption_fail_closed',
        'safe'
      );
      if (cleared) {
        hasAuthoritativeBlur = false;
      ...
      console.log(
        '[MW-MVP-REGULAR-HEAL-APPLY-BLUR-BLOCKED-UNRESOLVED-UNKNOWN-OWNERSHIP-V1] fail_closed_heal_apply_blur_blocked',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'contextKind=' + String(contextKind || 'none'),
        'cardNodeId=' + String(cardNodeId || 'none'),
      ...
      postNonShortsTransitionDiag('regular_main_heal_apply_blur_blocked_unresolved_unknown_ownership', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-HEAL-APPLY-BLUR-BLOCKED-UNRESOLVED-UNKNOWN-OWNERSHIP-V1',
        contextKind: String(contextKind || 'none'),
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
      ...
      const cleared = clearAllBlurAndOverlay(
        videoNode,
        normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
        'regular_heal_apply_blur_blocked_unresolved_unknown_ownership',
        'safe'
      );
      if (cleared) {
        hasAuthoritativeBlur = false;
    ...
    const overlayGeometryAnchoredToVideo = !!(
      videoOverlay &&
      videoOverlay.isConnected &&
      isNonShortsOverlayGeometryAnchored(videoOverlay, videoNode)
    );
    const keepOverlayDuringRegularUnresolvedChurn = !!(
      ...
      overlayWithinCard &&
      overlayGeometryAnchoredToVideo &&
    ...
    const regularUnresolvedSingleOverlayFallbackBlocked = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      isTransitionChurnReason &&
      !activeVideoBlurred &&
    ...
    if (regularUnresolvedSingleOverlayFallbackBlocked) {
      postNonShortsTransitionDiag('regular_main_overlay_singleton_fallback_blocked', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-OVERLAY-SINGLETON-FALLBACK-BLOCK-V1',
        overlayId: String((videoOverlay && videoOverlay.dataset && videoOverlay.dataset.mwOverlayId) || 'unknown'),
        overlayForItemKey: String(overlayForItemKey || 'unknown'),
    ...
    const pendingRevealOverlayOwnershipTrusted = !!(
      !!videoOverlay &&
      !!videoOverlay.isConnected &&
      !!videoOverlay.parentElement &&
      overlayWithinCard &&
    ...
    const nonShortsPendingRevealGuardLegacy = !!(
      revealFallbackScopeEligible &&
      !blockRegularFallbackNow &&
      !contextData &&
    ...
    const nonShortsPendingRevealGuard = !!(
      ...
      (
        ...
        (
          ...
          pendingRevealOverlayOwnershipTrusted
        )
      )
    );
    const resolvedContextKind = String(contextKind || 'none');
    const nonShortsResolvedContextOverlayHoldLegacy = !!(
      revealFallbackScopeEligible &&
      !!contextData &&
    ...
    const nonShortsResolvedContextOverlayHold = !!(
      nonShortsResolvedContextOverlayHoldLegacy &&
      resolvedContextKind === 'card_blurred'
    );
    if (
      nonShortsResolvedContextOverlayHoldLegacy &&
      ...
      postNonShortsTransitionDiag('regular_non_shorts_overlay_resolved_context_hold_blocked_global', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-OVERLAY-RESOLVED-CONTEXT-BLOCK-GLOBAL-V1',
    ...
    const nonShortsPositiveHoldOverlayHold = !!(
      regularPathQuarantinePassed &&
      !isHomeShortsShelfVideo &&
      unresolvedRegularTransitionNow &&
      regularPositiveHoldKeepBlur &&
      !regularUnresolvedNoCardWeakSameItemQuarantine &&
      !regularUnresolvedNoCardNoStrongOwnershipVeto &&
      !regularUnresolvedContextTerminalVetoActive &&
      !!videoOverlay &&
      !!videoOverlay.isConnected &&
      !!videoOverlay.parentElement &&
      overlayWithinCard &&
    ...
    if (
      ...
      !!videoOverlay &&
      !!videoOverlay.isConnected &&
      (!overlayGeometryAnchoredToVideo || !overlayWithinCard || cardOverlayCount > 1)
    ) {
      const overlayId = String((videoOverlay.dataset && videoOverlay.dataset.mwOverlayId) || 'unknown');
      removeRevealOverlay(
        videoNode,
        overlayProbeSrc || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
        'regular_unresolved_overlay_ownership_ambiguous'
      );
      videoOverlay = null;
      postNonShortsTransitionDiag('regular_main_overlay_ownership_ambiguous_removed', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-OVERLAY-OWNERSHIP-HARDEN-V1',
        overlayId: overlayId,
        overlayWithinCard: !!overlayWithinCard,
        overlayGeometryAnchored: !!overlayGeometryAnchoredToVideo,
        cardOverlayCount: Number(cardOverlayCount || 0),
      });
    }
    if (nonShortsPendingRevealGuardLegacy && !nonShortsPendingRevealGuard) {
      postNonShortsTransitionDiag('regular_pending_reveal_guard_veto_untrusted_intent', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-PENDING-REVEAL-GUARD-VETO-V1',
        pendingRevealActive: !!pendingRevealActive,
        videoLikelyPlaying: !!videoLikelyPlaying,
        trustedOverlayOwnership: !!pendingRevealOverlayOwnershipTrusted,
        action: 'do_not_hold_overlay',
      });
    }
    if (
      !activeVideoBlurred &&
      ...
      !nonShortsPositiveHoldOverlayHold &&
      videoOverlay &&
      videoOverlay.parentElement
    ) {
    ...
    } else if (nonShortsPositiveHoldOverlayHold && videoOverlay && videoOverlay.parentElement) {
      console.log(
        '[MW-MVP-REGULAR-THUMB-POSITIVE-HOLD-OVERLAY-VETO-V1] overlay_remove_veto_positive_hold',
        'reason=' + (reason || 'unknown'),
        'nodeId=' + videoNodeId,
        'overlayId=' + String((videoOverlay.dataset && videoOverlay.dataset.mwOverlayId) || 'unknown'),
        'proofKind=' + String(regularPositiveHoldProofKind || 'none'),
        'ttlRemaining=' + String(regularPositiveHoldTtlRemaining || 0),
        'action=keep_overlay'
      );
      postNonShortsTransitionDiag('regular_main_overlay_remove_veto_positive_hold', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-THUMB-POSITIVE-HOLD-OVERLAY-VETO-V1',
        overlayId: String((videoOverlay.dataset && videoOverlay.dataset.mwOverlayId) || 'unknown'),
        proofKind: String(regularPositiveHoldProofKind || 'none'),
        ttlRemaining: Number(regularPositiveHoldTtlRemaining || 0),
      });
    ...
    if (
      ...
      const regularNoContextHoldBlockedUnknownIdentity = !!(
        regularPathQuarantinePassed &&
        !isHomeShortsShelfVideo &&
        unresolvedRegularTransitionNow &&
        isTransitionChurnReason &&
      ...
      if (reasonText === 'event:loadeddata' && !activeVideoBlurred && videoNode.isConnected) {
        if (noContextPositiveOwnershipProof && !regularNoContextHoldBlockedUnknownIdentity) {
          ...
          if (blockedUnknownIdentity) {
            ...
            postNonShortsTransitionDiag('regular_no_context_hold_blocked_unknown_identity', {
              reason: String(reason || 'unknown'),
              nodeId: videoNodeId,
              marker: 'MW-MVP-REGULAR-NOCONTEXT-HOLD-BLOCKED-UNKNOWN-IDENTITY-V1',
              cardNodeId: String(cardNodeId || 'none'),
              strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
              reattachItemKey: String(reattachItemKey || 'unknown'),
              hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
          ...
          postNonShortsTransitionDiag('transition_decision', {
            ...
            action: 'clear_blur',
            marker: 'MW-MVP-REGULAR-NEGATIVE-TRANSITION-GUARD-V1',
            blockedNoProof: !blockedUnknownIdentity,
            blockedUnknownIdentity: !!blockedUnknownIdentity,
            cleared: !!cleared,
          });
        }
      }
    ...
    const unresolvedGlobalContextDecisionVeto = !!(
      isMainSurfaceUrl &&
      isHomeShortsShelfVideo &&
      String(contextKind || 'none') === 'itemkey_global' &&
      String(cardNodeId || 'none') === 'none' &&
    ...
    const unresolvedRegularTerminalDecisionFailClosed = !!(
      isMainSurfaceUrl &&
      !isHomeShortsShelfVideo &&
      unresolvedRegularTransitionNow &&
      isTransitionChurnReason &&
    ...
    let mvpTransitionAction = contextData
      ? 'apply_blur'
    ...
    if (unresolvedRegularTerminalDecisionFailClosed) {
      mvpTransitionAction = 'clear_blur';
      postNonShortsTransitionDiag('regular_terminal_decision_fail_closed_unknown_identity', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-TERMINAL-DECISION-FAIL-CLOSED-UNKNOWN-IDENTITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
    ...
    postNonShortsTransitionDiag('transition_decision', {
      ...
      unresolvedRegularTerminalDecisionFailClosed: !!unresolvedRegularTerminalDecisionFailClosed,
    });
    if (regularPositiveContinuitySignal && !(activeVideoBlurred || hasAuthoritativeBlur)) {
      ...
      const positiveDropUnresolved = !!(
        unresolvedRegularTransitionNow ||
        unresolvedShortsShelfTransitionNow ||
        unresolvedLaneFlipNoIdentityNow
      );
      if (positiveDropUnresolved) {
        incrementMvpTransitionCounter(
          'positiveTransitionBlurDroppedUnresolved',
          'reason=' + String(reason || 'unknown') +
          ' nodeId=' + String(videoNodeId || 'none') +
          ' contextKind=' + String(contextKind || 'none')
        );
      ...
      if (contextData) {
        incrementMvpTransitionCounter(
          'positiveTransitionBlurDroppedWithContext',
          'reason=' + String(reason || 'unknown') +
          ' nodeId=' + String(videoNodeId || 'none') +
          ' contextKind=' + String(contextKind || 'none')
        );
    ...
    // [Patch36] Skip the stale sweep for Shorts shelf videos (isHomeShortsShelfVideo=true).
    // When card=#content (the weak boundary the selector falls through to for grid-shelf
    // promoted players), card.querySelectorAll scans every overlay on the page. The sweep
    // would then remove overlays from OTHER positive thumbnails whose item keys are not in
    // resolvedOwnershipKeys for the current Short, wiping reveal UI across the whole page.
    // Shorts overlay lifecycle is handled by createRevealOverlay dedup and the portal system.
    if (!skipStaleSweepForUnresolvedNoise && card && typeof card.querySelectorAll === 'function' && !isHomeShortsShelfVideo) {
      const overlays = card.querySelectorAll('.mw-reveal-overlay');
      for (let i = 0; i < overlays.length; i += 1) {
        const overlay = overlays[i];
        if (!overlay || overlay.nodeType !== 1 || !overlay.isConnected) continue;
  ...
  function ensureNonShortsReattachListeners() {
    ...
    const onVideoLifecycleEvent = function(event) {
      ...
      const eventEpoch = Number(state.pageEpoch || 0);
      diagNonShortsReattach(target, 'event:' + String(event.type || 'unknown'), eventEpoch);
    };
  ...
  function isNodeClassificationLocked(element, reason) {
    ...
    const isRegularUnresolvedNoProofDecontaminate = typeof reason === 'string' &&
      reason.indexOf('regular_unresolved_no_proof_decontaminate') !== -1;
    ...
    if (isRegularUnresolvedNoProofDecontaminate) return false; // unresolved no-proof clear must be allowed
  ...
  function createRevealOverlay(element, src, category, itemId, allowShortsReresolve) {
    ...
    const nonShortsMainSurfaceOverlayProvenanceVeto = !!(
      !shortsMode &&
      isYouTubeMainPageThumbnailSurfaceUrl(window.location.href) &&
      isAuthoritativeHardBlur(element)
    );
    if (nonShortsMainSurfaceOverlayProvenanceVeto) {
      const resolvedOverlaySrc = normalizeUrl(String(src || (element.dataset && element.dataset.mwSrc) || '')) || '';
      const resolvedOverlayItemKey = getDiagItemKey(resolvedOverlaySrc || String(src || (element.dataset && element.dataset.mwSrc) || ''));
      ...
      const provenanceMatch = !!(
        (resolvedOverlaySrc && hardBlurSrc && resolvedOverlaySrc === hardBlurSrc) ||
        (
          resolvedOverlayItemKey &&
          resolvedOverlayItemKey !== 'unknown' &&
          hardBlurItemKey &&
          hardBlurItemKey !== 'unknown' &&
          resolvedOverlayItemKey === hardBlurItemKey
        )
      );
      if (!provenanceMatch) {
        incrementMvpTransitionCounter(
          ...
          ' overlayItemKey=' + String(resolvedOverlayItemKey || 'unknown') +
          ' hardBlurItemKey=' + String(hardBlurItemKey || 'unknown')
        );
        removeRevealOverlay(element, src, 'createRevealOverlay_provenance_veto');
        console.log(
          '[MW-MVP-REGULAR-OVERLAY-PROVENANCE-VETO-V1] overlay_create_veto_provenance_mismatch',
          'nodeId=' + String(getDiagNodeId(element) || 'none'),
          'overlayItemKey=' + String(resolvedOverlayItemKey || 'unknown'),
          'hardBlurItemKey=' + String(hardBlurItemKey || 'unknown'),
          'overlaySrc=' + String(resolvedOverlaySrc || '').substring(0, 180),
          'hardBlurSrc=' + String(hardBlurSrc || '').substring(0, 180)
        );
        return;
      }
  ...
  function handleModerationResult(message) {
    try {
      ...
      if (element && element.isConnected) {
        ...
        if (mvpMainSurface && finalBlur) {
          const resultCardNode = findNonShortsStrongCardNode(element) || findRegularMainCardFallbackNode(element);
          const resultCardNodeId = resultCardNode ? getDiagNodeId(resultCardNode) : 'none';
          ...
          const negativeLatchRead = readRegularMainCardNegativeLatch(
            String(resultCardNodeId || 'none'),
            String(resultItemKey || 'unknown'),
            String(normalizedResultSrc || '')
          );
          const negativeProofVeto = !!(localKnownNegative || negativeLatchRead.matched);
          if (negativeProofVeto) {
            finalBlur = false;
            decisionReason = (decisionReason ? decisionReason + '/' : '') + 'regular_negative_latch_veto';
            console.log(
              '[MW-MVP-REGULAR-NEGATIVE-LATCH-VETO-V1]',
              'itemId=' + String(itemId || 'none'),
              'nodeId=' + getDiagNodeId(element),
              'cardNodeId=' + String(resultCardNodeId || 'none'),
        ...
        if (finalBlur) {
          ...
          if (mvpMainSurface && element && element.dataset) {
            stampRegularMainNegativeProof(element, src, 'safe_result');
          }
      ...
      if (finalBlur) {
        clearSafeResolved(src);
        findAndBlur(src, predictedLabel || category, CONFIG.blurStrength, true, itemId, element);
      } else {
        markSafeResolved(src);
   ...
   */
  function findAndBlur(src, category, blurStrengthPx, shouldBlur, originItemId, originElement) {
    if (!shouldBlur) return;
    if (isRevealedForSource(src, null)) return;
    
    try {
      const regularMainSurface = !isShortsModeActive() && isYouTubeMainPageThumbnailSurfaceUrl(window.location.href);
      let originCardNodeId = 'none';
      if (regularMainSurface) {
        const originNode = (
          (originElement && originElement.nodeType === 1 ? originElement : null) ||
          (originItemId && state.elements && state.elements.get ? state.elements.get(originItemId) : null)
        );
        if (originNode && originNode.nodeType === 1) {
          const originCard = findNonShortsStrongCardNode(originNode) || findRegularMainCardFallbackNode(originNode);
          originCardNodeId = originCard ? getDiagNodeId(originCard) : 'none';
        }
      }
      const shouldAllowFanoutNode = function(node) {
        if (!regularMainSurface) return true;
        if (!node || node.nodeType !== 1) return false;
        if (!originCardNodeId || originCardNodeId === 'none') return false;
        const nodeCard = findNonShortsStrongCardNode(node) || findRegularMainCardFallbackNode(node);
        const nodeCardNodeId = nodeCard ? getDiagNodeId(nodeCard) : 'none';
        if (!nodeCardNodeId || nodeCardNodeId === 'none') return false;
        return nodeCardNodeId === originCardNodeId;
      };
      let fanoutCandidates = 0;
      let fanoutApplied = 0;
      let fanoutRecycled = 0;
      let fanoutNewlyDiscovered = 0;
      let fanoutBlockedByCard = 0;
      let fanoutBlockedByNegativeProof = 0;
      const normalizedFanoutSrc = normalizeUrl(String(src || '')) || '';
      const fanoutItemKey = getDiagItemKey(normalizedFanoutSrc || String(src || ''));
      ...
      const shouldBlockByNegativeProof = function(node) {
        if (!regularMainSurface || !node || node.nodeType !== 1) return false;
        const localKnownNegative = String((node.dataset && node.dataset.mwKnownNegativeForCardOrItem) || '') === '1';
        const nodeCard = findNonShortsStrongCardNode(node) || findRegularMainCardFallbackNode(node);
        const nodeCardNodeId = nodeCard ? getDiagNodeId(nodeCard) : 'none';
        const negativeLatchRead = readRegularMainCardNegativeLatch(
          String(nodeCardNodeId || 'none'),
          String(fanoutItemKey || 'unknown'),
          String(normalizedFanoutSrc || '')
        );
        return !!(localKnownNegative || negativeLatchRead.matched);
      };

      // Images
      document.querySelectorAll('img').forEach(img => {
        if ((img.src === src || img.dataset.mwOrigSrc === src) && !isRevealedForSource(src, img)) {
          if (!shouldAllowFanoutNode(img)) {
            fanoutBlockedByCard += 1;
            return;
          }
          if (shouldBlockByNegativeProof(img)) {
            fanoutBlockedByNegativeProof += 1;
            return;
          }
          trackFanoutNode(img);
      ...
      document.querySelectorAll('video').forEach(video => {
        if ((video.poster === src || video.dataset.mwOrigPoster === src) && !isRevealedForSource(src, video)) {
          if (!shouldAllowFanoutNode(video)) {
            fanoutBlockedByCard += 1;
            return;
          }
          if (shouldBlockByNegativeProof(video)) {
            fanoutBlockedByNegativeProof += 1;
            return;
          }
      ...
      document.querySelectorAll('[data-mw-bg-src]').forEach(el => {
        if (el.dataset.mwBgSrc === src && !isRevealedForSource(src, el)) {
          if (!shouldAllowFanoutNode(el)) {
            fanoutBlockedByCard += 1;
            return;
          }
          if (shouldBlockByNegativeProof(el)) {
            fanoutBlockedByNegativeProof += 1;
            return;
          }
          trackFanoutNode(el);
      ...
      console.log(
        ...
        'fanoutBlockedByCard=' + fanoutBlockedByCard,
        'fanoutBlockedByNegativeProof=' + fanoutBlockedByNegativeProof,
        'fanoutRecycled=' + fanoutRecycled,
        'fanoutNewlyDiscovered=' + fanoutNewlyDiscovered,
        'originCardNodeId=' + String(originCardNodeId || 'none'),
        'navId=' + NAV_ID,
        'pageEpoch=' + state.pageEpoch,
        'urlFamily=' + getDiagUrlFamily(window.location.href)
      );
  ...
  function setupMutationObserver(root) {
    ...
    const observer = new MutationObserver(mutations => {
      ...
      const observerEpochSnapshot = Number(state.pageEpoch || 0);
      ...
      for (const mutation of mutations) {
        ...
        mutation.addedNodes.forEach(node => {
          ...
          if (isYouTube() && !isShortsModeActive()) {
            if (String(node.tagName || '').toUpperCase() === 'VIDEO') {
              diagNonShortsReattach(node, 'mutation_added:video', observerEpochSnapshot);
            }
            if (typeof node.querySelectorAll === 'function') {
              node.querySelectorAll('video').forEach(videoNode => {
                diagNonShortsReattach(videoNode, 'mutation_added:descendant_video', observerEpochSnapshot);
              });
            }
          }
        ...
        if (mutation.type === 'attributes') {
          ...
          if (
            ...
            diagNonShortsReattach(target, 'attr:' + attr, observerEpochSnapshot);
          }
  ...
  // Startup rescans (trimmed to reduce homepage jank while keeping coverage for late mounts)
  const isYT = isYouTube();
  scheduleInitTimeout('initialFullScan', scanFullPage, 500);
  scheduleInitTimeout('initialFullScan', scanFullPage, 1800);
  
  if (isYT) {
    scheduleInitTimeout('initialYouTubeScan', scanYouTubeThumbnails, 1000);
    scheduleInitTimeout('initialYouTubeScan', scanYouTubeThumbnails, 2600);
  }

  function startYouTubePeriodicScan(reason) {
    if (!isYT || timerState.paused || timerState.youtubePeriodicInterval) return;
    // Regular homepage already has mutation + scroll + startup scans; skip this heavy
    // periodic sweep outside Shorts to reduce steady-state jank.
    if (!isShortsModeActive()) return;
  ...
  const checkUrlChange = () => {
    if (window.location.href !== lastUrl) {
      ...
      if (state.regularMainCardNegativeLatch && typeof state.regularMainCardNegativeLatch.clear === 'function') {
        state.regularMainCardNegativeLatch.clear();
      }
