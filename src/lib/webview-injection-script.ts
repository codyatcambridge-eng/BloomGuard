...
export function generateModerationScript(config: InjectionConfig): string {
  ...
  mwDiagLog('[Prayer1] startup_diag_marker');
  ...
  const persistedMainSurfaceLaneFlipUnknownIdentityQuarantineByNode = (function() {
    if (window.__MW_MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_BY_NODE__ && typeof window.__MW_MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_BY_NODE__.get === 'function') {
      return window.__MW_MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_BY_NODE__;
    }
    window.__MW_MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_BY_NODE__ = new WeakMap();
    return window.__MW_MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_BY_NODE__;
  })();
  ...
  const MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_MS = 900;
  ...
  function diagNonShortsReattach(videoNode, reason, transitionEpoch) {
    ...
    let laneFlipSameNodeUnknownIdentityQuarantineActive = false;
    const laneFlipUnknownIdentityQuarantineEntry = persistedMainSurfaceLaneFlipUnknownIdentityQuarantineByNode.get(videoNode) || null;
    if (
      laneFlipUnknownIdentityQuarantineEntry &&
      Number.isFinite(laneFlipUnknownIdentityQuarantineEntry.expiresAt) &&
      Number(laneFlipUnknownIdentityQuarantineEntry.expiresAt || 0) > Date.now()
    ) {
      laneFlipSameNodeUnknownIdentityQuarantineActive = true;
    } else if (laneFlipUnknownIdentityQuarantineEntry) {
      persistedMainSurfaceLaneFlipUnknownIdentityQuarantineByNode.delete(videoNode);
    }
    if (isMainSurfaceUrl) {
      ...
      if (
        ...
        const laneFlipSameNodeUnknownIdentity = !!(
          String(cardNodeId || 'none') === 'none' &&
          (!strictContinuityItemKey || strictContinuityItemKey === 'unknown') &&
        ...
        postNonShortsTransitionDiag('lane_flip_same_node', {
          ...
          laneFlipSameNodeUnknownIdentity: !!laneFlipSameNodeUnknownIdentity,
        });
        if (laneFlipSameNodeUnknownIdentity) {
          const quarantineUntil = nowMs + MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_MS;
          persistedMainSurfaceLaneFlipUnknownIdentityQuarantineByNode.set(videoNode, {
            setAt: nowMs,
            expiresAt: quarantineUntil,
            prevLane: String(previousLaneState.lane || 'unknown'),
            nextLane: String(nextLane || 'unknown'),
            reason: String(reason || 'unknown'),
          });
          ...
          if (videoNode && videoNode.dataset) {
            videoNode.dataset.mwRegularPositiveProof = '0';
            videoNode.dataset.mwRegularPositiveProofItemKey = 'unknown';
            videoNode.dataset.mwRegularPositiveProofSrc = '';
            videoNode.dataset.mwRegularPositiveProofNodeId = 'none';
            videoNode.dataset.mwRegularPositiveProofCardNodeId = 'none';
            videoNode.dataset.mwPendingRevealUntil = '0';
          }
          postNonShortsTransitionDiag('lane_flip_same_node_unknown_identity_quarantine_set', {
            reason: String(reason || 'unknown'),
            nodeId: videoNodeId,
            marker: 'MW-FLICKER-LANE-FLIP-SAME-NODE-UNKNOWN-IDENTITY-QUARANTINE-V1',
            prevLane: String(previousLaneState.lane || 'unknown'),
            nextLane: String(nextLane || 'unknown'),
            quarantineUntil: Number(quarantineUntil || 0),
            quarantineMs: Number(MAIN_SURFACE_LANE_FLIP_UNKNOWN_IDENTITY_QUARANTINE_MS || 0),
          });
        }
        if (
          laneFlipRegularToShortsNoCardIdentity &&
    ...
    let contextWasBlockedByStaleAdoption = false;
    ...
    if (isMainSurfaceUrl && card && typeof card.querySelector === 'function') {
      ...
      if (contextNode) {
        ...
        // When the context node IS the video node itself (self-referential lookup), the
        // card's hard-blur metadata is from a previous positive that occupied this slot.
        // Only adopt it if the live src still matches — otherwise a newly-loaded negative
        // would inherit the stale key, bypassing regularUnknownIdentityContextBlock.
        const cardContextSelfReferential = (contextNode === videoNode);
        const cardContextSrcMatchesLive = !!(
          cardContextHardSrc &&
          (
            (normalizedPoster && cardContextHardSrc === normalizedPoster) ||
            (normalizedCurrent && cardContextHardSrc === normalizedCurrent)
          )
        );
        ...
        if (strictCardMatch || strictUnknownButCardAuthoritative) {
          ...
          console.log(
            '[DIAG][NON_SHORTS_REATTACH] card_blurred_blocked_' + blockedReason,
            'reason=' + (reason || 'unknown'),
            'nodeId=' + videoNodeId,
            'selfReferential=' + String(!!cardContextSelfReferential),
            'srcMatchesLive=' + String(!!cardContextSrcMatchesLive),
            'contextWasBlockedByStaleAdoption=' + String(!!contextWasBlockedByStaleAdoption),
            'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
            'contextItemKey=' + String(contextItemKeyForCard || 'unknown'),
            'cardContextHardItemKey=' + String(cardContextHardItemKey || 'unknown')
          );
        }
      }
    }
    ...
    const earlyCurrentSrcForProof = normalizeUrl(String(normalizedPoster || normalizedCurrent || (videoNode.dataset && videoNode.dataset.mwSrc) || '')) || '';
    const earlyUnknownIdentityStartedAt = Number((videoNode.dataset && videoNode.dataset.mwTransitionUnknownIdentityStartedAt) || 0);
    const earlyUnknownIdentityHoldExpired = !!(
      Number.isFinite(earlyUnknownIdentityStartedAt) &&
      earlyUnknownIdentityStartedAt > 0 &&
      (Date.now() - earlyUnknownIdentityStartedAt) > 1400
    );
    const earlySameLocalSrcProof = !!(
      earlyRegularProofSrc &&
      earlyCurrentSrcForProof &&
      earlyRegularProofSrc === earlyCurrentSrcForProof
    );
    const regularUnknownIdentityPositiveStabilityDefer = !!(
      regularUnresolvedGhostState &&
      isMainSurfaceUrl &&
      !isHomeShortsShelfVideo &&
      earlyPreviousAuthoritativePositive &&
      earlySameLocalSrcProof &&
      !earlyUnknownIdentityHoldExpired &&
      (hasAuthoritativeBlur || hasIncidentalBlur)
    );
    const regularUnknownIdentityTerminalAuthorityActive = !!(
      regularUnresolvedGhostState &&
      !isHomeShortsShelfVideo &&
      !contextData &&
      String(cardNodeId || 'none') === 'none' &&
    ...
    if (
      ...
      !regularUnknownIdentityPositiveStabilityDefer &&
      !regularUnknownIdentityTerminalAuthorityActive &&
      (
    ...
    } else if (regularUnknownIdentityPositiveStabilityDefer) {
      postNonShortsTransitionDiag('regular_main_unknown_identity_clear_deferred_positive_stability', {
        reason: String(reason || 'unknown'),
        nodeId: videoNodeId,
        marker: 'MW-MVP-REGULAR-UNKNOWN-IDENTITY-CLEAR-DEFER-POSITIVE-STABILITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
    ...
    const regularCriticalTransitionReason = !!(
      reasonText === 'attr:src' ||
      reasonText === 'mutation_added:descendant_video' ||
      reasonText === 'event:loadeddata' ||
      reasonText === 'event:playing' ||
      reasonText === 'event:play'
    );
    const regularUnknownIdentityQuarantineNow = !!(
      unresolvedRegularTransitionNow &&
      regularCriticalTransitionReason &&
      (!strictIdentityKnown || strictContinuityItemKey === 'unknown') &&
      (!reattachItemKey || reattachItemKey === 'unknown')
    );
    if (regularUnknownIdentityQuarantineNow && videoNode && videoNode.dataset) {
      const startedAt = Number(videoNode.dataset.mwTransitionUnknownIdentityStartedAt || 0);
      if (!Number.isFinite(startedAt) || startedAt <= 0) {
        videoNode.dataset.mwTransitionUnknownIdentityStartedAt = String(Date.now());
      }
      videoNode.dataset.mwTransitionUnknownIdentity = '1';
    } else if (videoNode && videoNode.dataset && contextData) {
      videoNode.dataset.mwTransitionUnknownIdentity = '0';
      videoNode.dataset.mwTransitionUnknownIdentityStartedAt = '0';
    }
    const regularUnknownIdentityQuarantineLatched = !!(
      unresolvedRegularTransitionNow &&
      videoNode &&
      videoNode.dataset &&
      String(videoNode.dataset.mwTransitionUnknownIdentity || '0') === '1'
    );
    const regularUnknownIdentityQuarantineActive = !!(
      regularUnknownIdentityQuarantineNow ||
      regularUnknownIdentityQuarantineLatched
    );
    const regularNoCardUnknownIdentity = !!(
      regularUnknownIdentityQuarantineActive &&
      String(cardNodeId || 'none') === 'none'
    );
    const regularSameItemProofAllowed = !!(
      priorHardBlurProofSameItemKey &&
      !regularNoCardUnknownIdentity
    );
    const regularUnknownIdentityHardProof = !!(
      strictIdentityKnown ||
      (reattachItemKey && reattachItemKey !== 'unknown') ||
    ...
    const regularNuclearAPositiveProofVeto = !!(
      ...
      !contextWasBlockedByStaleAdoption &&
      (
        priorHardBlurProofSameNode ||
        priorHardBlurProofSameCard ||
        priorHardBlurProofSameItemKey ||
    ...
    const regularNuclearAUnknownIdentityNoBlur = !!(
      ...
      isTransitionChurnReason &&
      !contextData &&
      (String(cardNodeId || 'none') === 'none' || contextWasBlockedByStaleAdoption) &&
      (!strictContinuityItemKey || strictContinuityItemKey === 'unknown') &&
      (!reattachItemKey || reattachItemKey === 'unknown') &&
      !regularUnknownIdentityHardProof &&
      !regularNuclearAPositiveProofVeto
    );
    const regularUnknownIdentityClearDebounceActive = !!(
      unresolvedRegularTransitionNow &&
      !contextData &&
      String(cardNodeId || 'none') === 'none' &&
      (!strictContinuityItemKey || strictContinuityItemKey === 'unknown') &&
      ...
      videoNode &&
      videoNode.dataset &&
      Number(videoNode.dataset.mwRegularUnknownIdentityLastClearAt || 0) > 0 &&
      (Date.now() - Number(videoNode.dataset.mwRegularUnknownIdentityLastClearAt || 0)) < 220
    );
    if (regularNuclearAUnknownIdentityNoBlur) {
      if (regularUnknownIdentityTerminalAuthorityActive) {
        postNonShortsTransitionDiag('regular_main_nuclear_a_unknown_identity_deferred_terminal_authority', {
          reason: String(reason || 'unknown'),
          nodeId: String(videoNodeId || 'none'),
          marker: 'MW-MVP-REGULAR-NEGATIVE-NUCLEAR-A-NO-BLUR-V1',
          action: 'defer_clear_blur',
          cardNodeId: String(cardNodeId || 'none'),
          strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
          reattachItemKey: String(reattachItemKey || 'unknown'),
          hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
          authority: 'terminal_veto_unknown_identity',
        });
      } else
      if (regularUnknownIdentityPositiveStabilityDefer) {
        postNonShortsTransitionDiag('regular_main_nuclear_a_unknown_identity_deferred_positive_stability', {
          reason: String(reason || 'unknown'),
          nodeId: String(videoNodeId || 'none'),
          marker: 'MW-MVP-REGULAR-NEGATIVE-NUCLEAR-A-NO-BLUR-V1',
          action: 'defer_clear_blur',
      ...
      } else if (regularUnknownIdentityClearDebounceActive) {
        postNonShortsTransitionDiag('regular_main_nuclear_a_unknown_identity_deferred_clear_debounce', {
          reason: String(reason || 'unknown'),
          nodeId: String(videoNodeId || 'none'),
          marker: 'MW-MVP-REGULAR-NEGATIVE-NUCLEAR-A-NO-BLUR-V1',
          action: 'defer_clear_blur',
        ...
        if (nuclearACleared && videoNode && videoNode.dataset) {
          videoNode.dataset.mwRegularUnknownIdentityLastClearAt = String(Date.now());
        }
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwPendingRevealUntil = '0';
    ...
    if (regularSameItemProofAllowed) regularPositiveHoldProofKind = 'same_item_key';
    else if (priorHardBlurProofSameReattachKey) regularPositiveHoldProofKind = 'same_reattach_key';
    else if (priorHardBlurProofSameLocalSrc) regularPositiveHoldProofKind = 'same_local_src';
    ...
    else if (priorHardBlurProofSameNode) regularPositiveHoldProofKind = 'same_node_authoritative';
    else if (priorHardBlurProofSameCard) regularPositiveHoldProofKind = 'replacement_same_card';
    if (regularUnknownIdentityQuarantineActive && regularPositiveHoldProofKind === 'same_node_authoritative') {
      regularPositiveHoldProofKind = 'same_node_authoritative_blocked_unknown_identity';
    }
    if (regularPositiveHoldProofKind === 'none' && unresolvedPositiveHoldPrecedenceStrong) {
    ...
    const regularUnknownIdentitySameLocalSrcOnlyBlock = !!(
      unresolvedRegularTransitionNow &&
      previousAuthoritativePositive &&
      !hasAnyNegativeProof &&
      String(cardNodeId || 'none') === 'none' &&
    ...
    const regularUnknownIdentityChurnReason = !!(
      isTransitionChurnReason &&
      (
        reasonText.indexOf('attr:') === 0 ||
        reasonText.indexOf('mutation_added:') === 0
    ...
    const regularUnknownIdentityForDecisionFreeze = !!(
      unresolvedRegularTransitionNow &&
      String(cardNodeId || 'none') === 'none' &&
      (!strictContinuityItemKey || strictContinuityItemKey === 'unknown') &&
      (!reattachItemKey || reattachItemKey === 'unknown') &&
      !contextData &&
      regularUnknownIdentityChurnReason
    );
    let regularDecisionFreezeUntil = Number((videoNode && videoNode.dataset && videoNode.dataset.mwRegularDecisionFreezeUntil) || '0');
    let regularChurnLastAt = Number((videoNode && videoNode.dataset && videoNode.dataset.mwRegularUnknownIdentityChurnAt) || '0');
    if (regularUnknownIdentityForDecisionFreeze) {
      const freezeNow = Date.now();
      if (Number.isFinite(regularChurnLastAt) && regularChurnLastAt > 0 && (freezeNow - regularChurnLastAt) <= 260) {
        regularDecisionFreezeUntil = freezeNow + 260;
        if (videoNode && videoNode.dataset) {
          videoNode.dataset.mwRegularDecisionFreezeUntil = String(regularDecisionFreezeUntil);
        }
      }
      if (videoNode && videoNode.dataset) {
        videoNode.dataset.mwRegularUnknownIdentityChurnAt = String(freezeNow);
      }
    } else if (contextData && videoNode && videoNode.dataset) {
      videoNode.dataset.mwRegularDecisionFreezeUntil = '0';
      videoNode.dataset.mwRegularUnknownIdentityChurnAt = '0';
      regularDecisionFreezeUntil = 0;
    }
    const regularUnknownIdentityDecisionFreezeActive = !!(
      unresolvedRegularTransitionNow &&
      regularUnknownIdentityForDecisionFreeze &&
      previousAuthoritativePositive &&
      (hasAuthoritativeBlur || hasIncidentalBlur || activeVideoBlurred) &&
      Number.isFinite(regularDecisionFreezeUntil) &&
      regularDecisionFreezeUntil > Date.now()
    );
    const regularUnknownIdentityRequiresNegativeProofForClear = !!(
      unresolvedRegularTransitionNow &&
      !contextData &&
      String(cardNodeId || 'none') === 'none' &&
      (!strictContinuityItemKey || strictContinuityItemKey === 'unknown') &&
    ...
    const regularClearAllowedByProof = !!(
      unresolvedRegularTransitionNow &&
      !regularUnknownIdentitySameLocalSrcOnlyBlock &&
      !regularUnknownIdentityRequiresNegativeProofForClear &&
      !regularUnknownIdentityDecisionFreezeActive &&
    ...
    const regularUnknownIdentityKnownNegativeHardClear = !!(
      unresolvedRegularTransitionNow &&
      !isHomeShortsShelfVideo &&
      !contextData &&
      String(cardNodeId || 'none') === 'none' &&
    ...
    const regularTerminalVetoUnknownIdentity = !!(
      ...
      (
        ...
        !regularUnknownIdentityRequiresNegativeProofForClear &&
        !regularUnknownIdentityDecisionFreezeActive &&
        !regularUnknownIdentityClearDebounceActive &&
        !hasPositiveContinuityProofStrong &&
        String(cardNodeId || 'none') === 'none' &&
        (!strictContinuityItemKey || strictContinuityItemKey === 'unknown') &&
        (!reattachItemKey || reattachItemKey === 'unknown')
    ...
    if (unresolvedRegularTransitionNow && !regularClearAllowedByProof && hasPositiveContinuityProofStrong) {
      ...
      decisionFreezeActive: !!regularUnknownIdentityDecisionFreezeActive,
      action: 'keep_blur',
      srcLineage: String((regularProofSrc || currentResolvedSrcForProof || '')).substring(0, 180),
    });
    } else if (regularUnknownIdentityDecisionFreezeActive) {
      postNonShortsTransitionDiag('regular_clear_frozen_unknown_identity_churn', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-CLEAR-FROZEN-UNKNOWN-IDENTITY-CHURN-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        proofKind: String(regularPositiveHoldProofKind || 'none'),
        previousAuthoritativePositive: !!previousAuthoritativePositive,
        clearAllowedByProof: !!regularClearAllowedByProof,
        freezeUntil: Number(regularDecisionFreezeUntil || 0),
        action: 'keep_blur_fail_closed',
      });
    } else if (regularUnknownIdentitySameLocalSrcOnlyBlock) {
      postNonShortsTransitionDiag('regular_clear_proof_blocked_unknown_identity', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-CLEAR-PROOF-BLOCKED-UNKNOWN-IDENTITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        proofKind: String(regularPositiveHoldProofKind || 'none'),
    ...
    if (regularUnknownIdentityWeakProofContamRisk) {
      ...
      const terminalVetoCleared = clearAllBlurAndOverlay(
        videoNode,
        currentResolvedSrcForProof || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
        'regular_main_terminal_veto_unknown_identity',
        'safe'
      );
      if (terminalVetoCleared && videoNode && videoNode.dataset) {
        videoNode.dataset.mwRegularUnknownIdentityLastClearAt = String(Date.now());
      }
      if (videoNode && videoNode.dataset) {
        videoNode.dataset.mwPendingRevealUntil = '0';
      ...
      activeVideoBlurred = false;
      console.log(
        '[MW-MVP-REGULAR-TERMINAL-VETO-UNKNOWN-IDENTITY-V1] veto_apply_blur_terminal',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'cardNodeId=' + String(cardNodeId || 'none'),
        'strictContinuityItemKey=' + String(strictContinuityItemKey || 'unknown'),
        'reattachItemKey=' + String(reattachItemKey || 'unknown'),
        'proofKind=' + String(regularPositiveHoldProofKind || 'none'),
        'cleared=' + String(!!terminalVetoCleared)
      );
      postNonShortsTransitionDiag('regular_main_terminal_veto_unknown_identity', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-TERMINAL-VETO-UNKNOWN-IDENTITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
        reattachItemKey: String(reattachItemKey || 'unknown'),
        hardBlurItemKey: String(hardBlurItemKey || 'unknown'),
        ...
        action: 'clear_blur',
        cleared: !!terminalVetoCleared,
      });
    } else if (unresolvedRegularTransitionNow && regularUnknownIdentityDecisionFreezeActive) {
      postNonShortsTransitionDiag('regular_main_terminal_veto_deferred_unknown_identity_churn', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-MVP-REGULAR-TERMINAL-VETO-UNKNOWN-IDENTITY-V1',
        cardNodeId: String(cardNodeId || 'none'),
    ...
    const shortsShelfUnknownIdentityQuarantineActive = !!(
      unresolvedShortsShelfTransitionNow &&
      (!strictIdentityKnown || strictContinuityItemKey === 'unknown') &&
      (!reattachItemKey || reattachItemKey === 'unknown')
    );
    const shortsShelfNoCardUnknownIdentity = !!(
      shortsShelfUnknownIdentityQuarantineActive &&
      String(cardNodeId || 'none') === 'none'
    );
    const shortsShelfSameItemProofAllowed = !!(
      shortsPriorProofSameItemKey &&
      !shortsShelfNoCardUnknownIdentity
    );
    const shortsShelfHardProof = !!(
      (strictContinuityItemKey && strictContinuityItemKey !== 'unknown') ||
      (reattachItemKey && reattachItemKey !== 'unknown') ||
    ...
    if (shortsShelfUnknownIdentityQuarantineActive && shortsShelfProofKind === 'same_node_authoritative') {
      shortsShelfProofKind = 'same_node_authoritative_blocked_unknown_identity';
    }
    ...
    if (activeVideoBlurred && hasAuthoritativeBlur) {
      ...
      const quarantineCleared = clearAllBlurAndOverlay(
        videoNode,
        normalizedCurrent || normalizedPoster || String((videoNode.dataset && videoNode.dataset.mwSrc) || ''),
        'regular_main_lane_flip_unknown_identity_quarantine_apply_veto',
        'safe'
      );
      hasAuthoritativeBlur = false;
      hasIncidentalBlur = false;
      activeVideoBlurred = false;
      holdBlurDuringUnresolvedTransition = false;
      postNonShortsTransitionDiag('regular_main_lane_flip_unknown_identity_quarantine_apply_veto', {
        reason: String(reason || 'unknown'),
        nodeId: String(videoNodeId || 'none'),
        marker: 'MW-FLICKER-LANE-FLIP-SAME-NODE-UNKNOWN-IDENTITY-QUARANTINE-V1',
        strictContinuityItemKey: String(strictContinuityItemKey || 'unknown'),
      ...
      console.log(
        '[MW-FLICKER-LANE-FLIP-SAME-NODE-UNKNOWN-IDENTITY-QUARANTINE-V1] apply_veto',
        'reason=' + String(reason || 'unknown'),
        'nodeId=' + String(videoNodeId || 'none'),
        'cleared=' + String(!!quarantineCleared)
      );
    } else if (contextData) {
    ...
    const regularContextApplyVetoByNegative = !!(
      !isHomeShortsShelfVideo &&
      !!contextData &&
      knownNegativeForCardOrItem
    );
    ...
    postNonShortsTransitionDiag('transition_decision', {
      ...
      laneFlipSameNodeUnknownIdentityQuarantineActive: !!laneFlipSameNodeUnknownIdentityQuarantineActive,
      regularUnknownIdentityKnownNegativeHardClear: !!regularUnknownIdentityKnownNegativeHardClear,
      regularContextApplyVetoByNegative: !!regularContextApplyVetoByNegative,
    });
