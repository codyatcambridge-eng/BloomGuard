...
export function generateModerationScript(config: InjectionConfig): string {
  ...
  const CONFIG = {
    ...
    nativeRevealOverlay: ${config.nativeRevealOverlay ? 'true' : 'false'},
  ...
  if (DOM_OVERLAY_ENABLED && !window.__MW_BLUR_NAV_HOOKED__) {
    ...
    history.pushState = function() {
      ...
      const currentUrl = String(window.location && window.location.href ? window.location.href : '');
      if (currentUrl.indexOf('youtube.com/shorts/') !== -1) {
        return result;
      }
  ...
  const state = {
    ...
    shortsHighRisk: new Set(), // src values currently considered high-risk in Shorts mode
    blurred: new Set(),
  ...
  function getShortsHighRiskKey(src) {
  ...
  function getUnsafeMaxScore(scores) {
    if (!scores || typeof scores !== 'object') return 0;
    const porn = safeScore(scores.porn ?? scores.Porn);
    const sexy = safeScore(scores.sexy ?? scores.Sexy);
    const hentai = safeScore(scores.hentai ?? scores.Hentai);
    return Math.max(porn, sexy, hentai);
  }
  ...
  function isKnownShortsHighRisk(src) {
    const key = getShortsHighRiskKey(src);
    if (!key) return false;
    return state.shortsHighRisk.has(key);
  }

  function hasKnownShortsHighRisk() {
    return state.shortsHighRisk.size > 0;
  }

  function markShortsHighRisk(src, reason) {
    const key = getShortsHighRiskKey(src);
    if (!key) return;
    if (state.shortsHighRisk.has(key)) return;
    state.shortsHighRisk.add(key);
    if (isShortsModeActive()) {
      startShortsReelTransitionObserver('high_risk_marked');
    }
    if (DIAG_YT_BLUR) {
      console.log(
        '[MW-YT][DIAG][SHORTS_FORCE]',
        'action=mark_high_risk',
        'src=' + key.substring(0, 180),
        'count=' + state.shortsHighRisk.size,
        'reason=' + (reason || 'unknown')
      );
    }
  }

  function clearShortsHighRisk(src, reason) {
    const key = getShortsHighRiskKey(src);
    if (!key) return;
    if (!state.shortsHighRisk.has(key)) return;
    state.shortsHighRisk.delete(key);
    if (state.shortsHighRisk.size <= 0) {
      stopShortsReelTransitionObserver('high_risk_cleared');
    }
    if (DIAG_YT_BLUR) {
      console.log(
        '[MW-YT][DIAG][SHORTS_FORCE]',
  ...
  function clearAllShortsHighRisk(reason) {
    if (state.shortsHighRisk.size <= 0) return;
    const clearedCount = state.shortsHighRisk.size;
    state.shortsHighRisk.clear();
    stopShortsReelTransitionObserver('high_risk_cleared_all');
    if (DIAG_YT_BLUR) {
      console.log(
        '[MW-YT][DIAG][SHORTS_FORCE]',
        'action=clear_all_high_risk',
        'count=' + clearedCount,
        'reason=' + (reason || 'unknown')
      );
    }
  ...
  function handleShieldAction(action) {
    ...
    if (action === 'false_positive') {
      ...
      forceGlobalUnblur('shield_false_positive');
      return;
    }
    const target = lastShieldTarget;
    if (!target || !target.src) {
  ...
  let batchQueue = [];
  let batchTimer = null;
  function normalizeNavId(value) {
    if (value === null || value === undefined) return null;
    const normalized = String(value).trim();
    return normalized ? normalized : null;
  }

  function navIdsMatch(left, right) {
    const leftNorm = normalizeNavId(left);
    const rightNorm = normalizeNavId(right);
    return leftNorm !== null && rightNorm !== null && leftNorm === rightNorm;
  }

  const NAV_ID = normalizeNavId(window.__MW_NAV_ID__) || normalizeNavId(window.__MW_HOST_NAV_ID__) || ('mw_' + Date.now().toString(36));
  window.__MW_NAV_ID__ = NAV_ID;
  function getCurrentNavId() {
    return normalizeNavId(window.__MW_HOST_NAV_ID__) || NAV_ID;
  }
  ...
  const timerState = {
    ...
    shortsReelTransitionObserver: null,
    shortsReelTransitionRoot: null,
    initialTimeouts: [],
  ...
  const SHORTS_STABLE_CONTAINER_SELECTORS = [
    '#shorts-player .short-video-container[aria-hidden="false"]',
    '#shorts-player .short-video-container[selected]',
    '#shorts-player .short-video-container',
  ...
  const SHORTS_STABLE_CONTAINER_TAG_SELECTOR = '.short-video-container, ytm-reel-video-renderer, ytm-shorts-lockup-view-model, #shorts-player';
  ...
  let shortsReelTransitionLastRearmAt = 0;
  ...
  function isRelevantShortsContainerNode(node) {
    ...
    if (element.classList && element.classList.contains('short-video-container')) return true;
  ...
  function diagLogLifecycleSnapshot(eventName, reason, previousUrl, nextUrl) {
    ...
    if (timerState.shortsReelTransitionObserver) activeTimerNames.push('shortsReelTransitionObserver');
  ...
  function isAdaptiveShortsScopeNode(node, root) {
    ...
    if (typeof element.closest === 'function') {
      return !!element.closest('#shorts-player, .short-video-container, ytm-reel-video-renderer, ytm-shorts-lockup-view-model');
    }
    return false;
  }
  ...
  function refreshAdaptiveShortsOverlayWatch(reason) {
    ...
    const observer = new MutationObserver(function(mutations) {
      ...
      if (identityChanged) {
        ...
        clearAllRevealOverlays('active_shorts_item_changed', true);
        clearAllShortsHighRisk('active_shorts_item_changed');
        resetShortsBlurContext('active_shorts_item_changed');
  ...
  function refreshShortsFreshnessOnReentry(reason, options) {
    ...
    startShortsReelTransitionObserver('reentry:' + (reason || 'unknown'));
    forceRearmShortsShield(
      'reentry:' + (reason || 'unknown'),
      nextCandidate && nextCandidate.container ? nextCandidate.container : null
    );
    return watchBound;
  }

  function resolveShortsRearmSource(node) {
    if (!node || node.nodeType !== 1) return '';
    const source = getDiagSourceFields(node);
    const attrSrc = node.getAttribute ? String(node.getAttribute('src') || '') : '';
    const attrPoster = node.getAttribute ? String(node.getAttribute('poster') || '') : '';
    const datasetSrc = node.dataset ? String(
      node.dataset.mwSrc ||
      node.dataset.mwOrigSrc ||
      node.dataset.mwOrigPoster ||
      node.dataset.mwBgSrc ||
      node.dataset.mwLastScanSrc ||
      ''
    ) : '';
    return normalizeUrl(source.currentSrc || source.poster || attrPoster || attrSrc || datasetSrc || '');
  }

  function collectShortsRearmTargets(container) {
    const targets = [];
    if (!container || container.nodeType !== 1) return targets;
    const tag = String(container.tagName || '').toUpperCase();
    if (tag === 'VIDEO' || tag === 'IMG') {
      targets.push(container);
    }
    if (typeof container.querySelectorAll === 'function') {
      const nodes = container.querySelectorAll('video.html5-main-video, video, img');
      for (let i = 0; i < nodes.length; i += 1) {
        targets.push(nodes[i]);
      }
    }
    if (targets.length === 0) {
      const fallback = document.querySelector('#shorts-player video.html5-main-video, #shorts-player video, #shorts-player img');
      if (fallback && fallback.nodeType === 1) {
  ...
  function forceRearmShortsShield(reason, candidateNode) {
    if (!FORCE_SHIELD_ON_SHORTS || !isShortsModeActive() || !hasKnownShortsHighRisk()) return false;
    if (timerState.paused || timerState.teardownDone) return false;
    const now = Date.now();
    if ((now - shortsReelTransitionLastRearmAt) < SHORTS_REEL_TRANSITION_REARM_COOLDOWN_MS) {
      return false;
    }

    const candidateContainer = getShortsCardOrPlayerContainerFromNode(candidateNode);
    const activeContainer = (
      candidateContainer && candidateContainer.isConnected
        ? candidateContainer
        : getActiveShortsPlayerContainer()
    );
    if (!activeContainer || !activeContainer.isConnected) return false;

    const targets = collectShortsRearmTargets(activeContainer);
    const seenSources = new Set();
    let armedCount = 0;
    for (let i = 0; i < targets.length; i += 1) {
      const target = targets[i];
      if (!target || target.nodeType !== 1 || !target.isConnected) continue;
      const src = resolveShortsRearmSource(target);
      if (!src || seenSources.has(src)) continue;
      if (!isKnownShortsHighRisk(src)) continue;
      seenSources.add(src);
      if (state.revealed.has(src) || target.dataset.mwRevealed === 'true') continue;
      clearSafeResolved(src);
      state.scanned.delete(src);
      applyBlur(
        target,
        src,
        'shorts_transition_guard',
        CONFIG.blurStrength,
        target.dataset.mwItemId || ''
      );
      armedCount += 1;
    }

    if (armedCount > 0) {
      shortsReelTransitionLastRearmAt = now;
      queueMutationScan(activeContainer, 'shorts_reel_transition_rearm:' + (reason || 'unknown'));
      triggerAdaptiveShortsTargetedRescan(
        'shorts_reel_transition_rearm:' + (reason || 'unknown'),
        null,
        { force: true, immediate: true }
      );
      scheduleShortsRevealOverlayReposition('shorts_reel_transition_rearm');
      if (DIAG_YT_BLUR) {
        console.log(
          '[MW-YT][DIAG][SHORTS_REEL]',
          'event=rearm_applied',
          'reason=' + (reason || 'unknown'),
          'armedCount=' + armedCount,
          'containerNodeId=' + getDiagNodeId(activeContainer)
        );
      }
      return true;
    }
  ...
  function stopShortsReelTransitionObserver(reason) {
    if (!timerState.shortsReelTransitionObserver) return;
    try {
      timerState.shortsReelTransitionObserver.disconnect();
    } catch (e) {}
    timerState.shortsReelTransitionObserver = null;
    timerState.shortsReelTransitionRoot = null;
    if (DIAG_YT_BLUR) {
      console.log(
        '[MW-YT][DIAG][SHORTS_REEL]',
        'event=observer_stop',
  ...
  function startShortsReelTransitionObserver(reason) {
    if (timerState.paused || timerState.teardownDone) return false;
    if (!FORCE_SHIELD_ON_SHORTS || !isShortsModeActive() || !hasKnownShortsHighRisk()) {
      stopShortsReelTransitionObserver('inactive:' + (reason || 'unknown'));
      return false;
    }
    const root = document.getElementById('shorts-player') || getActiveShortsPlayerContainer();
    if (!root || !root.isConnected) return false;
    if (
      timerState.shortsReelTransitionObserver &&
      timerState.shortsReelTransitionRoot === root
    ) {
      return true;
    }

    stopShortsReelTransitionObserver('refresh:' + (reason || 'unknown'));

    const observer = new MutationObserver(function(mutations) {
      if (!FORCE_SHIELD_ON_SHORTS || !isShortsModeActive() || !hasKnownShortsHighRisk()) return;
      ...
      for (let i = 0; i < mutations.length; i += 1) {
        ...
        if (mutation.type === 'childList') {
          for (let j = 0; j < mutation.addedNodes.length; j += 1) {
            ...
            const matchesContainer = isRelevantShortsContainerNode(node);
            const hasReelDescendant = typeof node.querySelector === 'function'
              ? !!node.querySelector('ytm-reel-video-renderer, ytm-shorts-lockup-view-model, video.html5-main-video, #shorts-player')
              : false;
            if (matchesContainer || hasReelDescendant) {
              shouldRearm = true;
              candidateNode = node;
              if (reasonParts.length < 3) reasonParts.push('child_added');
            }
          ...
          for (let j = 0; j < mutation.removedNodes.length; j += 1) {
            ...
            if (!node || node.nodeType !== 1) continue;
            if (isRelevantShortsContainerNode(node)) {
              shouldRearm = true;
              candidateNode = mutation.target && mutation.target.nodeType === 1
                ? mutation.target
                : candidateNode;
              if (reasonParts.length < 3) reasonParts.push('child_removed');
            }
          ...
          const inShortsScope = (
            isRelevantShortsContainerNode(target) ||
            !!getShortsCardOrPlayerContainerFromNode(target)
          );
          if (!inShortsScope) continue;
          const attrSignalsTransition = (
            attr === 'aria-hidden' ||
            attr === 'aria-current' ||
            attr === 'hidden' ||
            attr === 'selected' ||
          ...
          if (!attrSignalsTransition) continue;
          shouldRearm = true;
          candidateNode = target;
          if (reasonParts.length < 3) reasonParts.push('attr:' + attr);
        }
      }

      if (!shouldRearm) return;
      const reasonSummary = reasonParts.length ? reasonParts.join(',') : 'mutation';
      forceRearmShortsShield('reel_transition:' + reasonSummary, candidateNode);
    });
    ...
    observer.observe(root, {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: SHORTS_REEL_TRANSITION_ATTRIBUTE_FILTER,
    });
    timerState.shortsReelTransitionObserver = observer;
    timerState.shortsReelTransitionRoot = root;
    forceRearmShortsShield('reel_observer_start:' + (reason || 'unknown'), root);
    if (DIAG_YT_BLUR) {
      console.log(
        '[MW-YT][DIAG][SHORTS_REEL]',
  ...
  function findStableShortsContainerByParentWalk(node) {
    ...
    while (current && current.nodeType === 1) {
      if (current.classList && current.classList.contains('short-video-container')) {
        if (current.isConnected) return { target: current, selectorUsed: 'parent_walk:.short-video-container' };
      }
  ...
  function runShortsHealthHealForContainer(container, reason) {
    ...
    if (!healTriggered) {
      ...
      const liveSafeResolved = !!(
        liveSrc &&
        !isKnownShortsHighRisk(liveSrc) &&
        isSafeResolvedActive(liveSrc)
      );
  ...
  function ensureRevealOverlayForBlurSignal(element, src, category, itemId, reason) {
    if (!element || element.nodeType !== 1 || !element.isConnected) return false;
    const targetSrc = src || (element.dataset ? element.dataset.mwSrc || '' : '');
    if (!targetSrc) return false;
    if (state.revealed.has(targetSrc) || element.dataset.mwRevealed === 'true') return false;

    let overlay = findRevealOverlayForElement(element, targetSrc);
    const hasRevealButton = !!(
      overlay &&
      overlay.isConnected &&
      typeof overlay.querySelector === 'function' &&
      overlay.querySelector('.mw-reveal-btn')
    );
    if (hasRevealButton) return false;
    ...
    createRevealOverlay(
      element,
      targetSrc,
      category || (element.dataset ? element.dataset.mwCategory || 'flagged' : 'flagged'),
      itemId || (element.dataset ? element.dataset.mwItemId || '' : '')
    );
    if (CONFIG.debug) {
      console.log(
        '[MW][RevealOverlay] rehydrated',
        'reason=' + (reason || 'blur_signal'),
        'node=' + getDiagNodeId(element),
        'src=' + String(targetSrc || '').substring(0, 120)
      );
    }
    return true;
  }
  ...
  function clearAllRevealOverlays(reason, clearRevealState) {
    const cleanupReason = reason || 'unknown';
    cancelShortsRevealOverlayReposition('clear_all_reveal_overlays:' + cleanupReason);
    let removedCount = 0;
    const overlays = document.querySelectorAll('.mw-reveal-overlay');
    for (let i = 0; i < overlays.length; i += 1) {
      const overlay = overlays[i];
    ...
    const overlayOwners = document.querySelectorAll('[data-mw-has-overlay="true"]');
    for (let i = 0; i < overlayOwners.length; i += 1) {
      const owner = overlayOwners[i];
      if (!owner || !owner.dataset) continue;
      owner.dataset.mwHasOverlay = 'false';
    }

    if (clearRevealState) {
      state.revealed.clear();
      const revealedNodes = document.querySelectorAll('[data-mw-revealed="true"]');
      for (let i = 0; i < revealedNodes.length; i += 1) {
    ...
    if (DIAG_YT_BLUR || removedCount > 0 || clearRevealState) {
      console.log(
        '[DIAG][REVEAL_UI] clear_all',
        'removed=' + removedCount,
        'clearRevealState=' + (!!clearRevealState),
  ...
  function forceGlobalUnblur(reason) {
    const actionReason = reason || 'manual_global_unblur';
    const navId = getCurrentNavId();
    const revealedSrcs = new Set();
    let clearedNodes = 0;
    let removedOverlayNodes = 0;

    state.pending.forEach(function(pendingItem, itemId) {
      if (!pendingItem) return;
      const pendingSrc = pendingItem.src || '';
      if (pendingSrc) revealedSrcs.add(pendingSrc);
      const pendingElement = pendingItem.element && pendingItem.element.isConnected
        ? pendingItem.element
        : state.elements.get(itemId);
      if (pendingElement && pendingElement.nodeType === 1) {
        if (clearAllBlurAndOverlay(pendingElement, pendingSrc, actionReason, 'revealed')) {
          clearedNodes += 1;
        }
        pendingElement.dataset.mwRevealed = 'true';
      }
      clearPendingItem(itemId, 'force_global_unblur');
    });

    const candidateNodes = document.querySelectorAll('[data-mw-moderated], .mw-softblur, .mw-blurred');
    candidateNodes.forEach(function(node) {
      if (!node || node.nodeType !== 1) return;
      const element = node;
      const srcForClear = element.dataset.mwSrc || '';
      if (srcForClear) revealedSrcs.add(srcForClear);
      if (clearAllBlurAndOverlay(element, srcForClear, actionReason, 'revealed')) {
        clearedNodes += 1;
      }
    ...
    document.querySelectorAll('.mw-reveal-overlay').forEach(function(overlayNode) {
      try {
        overlayNode.remove();
        removedOverlayNodes += 1;
      } catch (e) {}
    });
    ...
    revealedSrcs.forEach(function(src) {
      if (!src) return;
      state.revealed.add(src);
      markSafeResolved(src);
    });

    setOverlayEnabled(false, actionReason + '_overlay_drop');
    console.log(
      '[MW][Unblur][Global]',
      'reason=' + actionReason,
      'navId=' + navId,
      'clearedNodes=' + clearedNodes,
      'removedOverlays=' + removedOverlayNodes,
      'revealedSrcs=' + revealedSrcs.size
    );
  }
    ...
    if (shortsMode) {
      ...
      if (alreadyHardBlurred) {
        ...
        if (element.isConnected) {
          ensureRevealOverlayForBlurSignal(
            element,
            src,
            category || 'flagged',
            itemId,
  ...
  function emitRevealLabelRequest(element, src, category, itemId) {
  ...
  function revealBlurredElement(element, src, category, itemId, overlayId, itemKey, triggerReason) {
    if (!element || element.nodeType !== 1) return false;
    if (!src) return false;
    if (state.revealed.has(src) || element.dataset.mwRevealed === 'true') {
      return false;
    }
    const revealReason = triggerReason || 'overlay_button';
    ...
    const labelItemId = emitRevealLabelRequest(element, src, category, itemId);
    showCorrectionOverlay(element, src, category, labelItemId);
    return true;
  }

  function selectNativeRevealCandidate() {
    const candidates = document.querySelectorAll('[data-mw-moderated="blurred"][data-mw-src]');
    ...
    for (let i = 0; i < candidates.length; i += 1) {
      const candidate = candidates[i];
      if (!candidate || candidate.nodeType !== 1 || !candidate.isConnected) continue;
      if (candidate.dataset.mwRevealed === 'true') continue;
      let rect = null;
      try {
      ...
      if (!rect) continue;
      const visibleRect = getVisibleViewportRect(rect, viewportWidth, viewportHeight);
      if (visibleRect.width < 16 || visibleRect.height < 16) continue;
      ...
      const dist = Math.abs(centerX - viewportCenterX) + Math.abs(centerY - viewportCenterY);
  ...
  function runNativeRevealRequest(reason) {
    const candidate = selectNativeRevealCandidate();
    if (!candidate) {
      return { ok: false, reason: 'no_blurred_candidate' };
    }
    const src = candidate.dataset.mwSrc || candidate.dataset.mwOrigSrc || '';
    if (!src) {
      return { ok: false, reason: 'missing_src' };
    }
    const category = candidate.dataset.mwCategory || 'flagged';
    const itemId = candidate.dataset.mwItemId || '';
    const itemKey = getDiagItemKey(src);
    const applied = revealBlurredElement(
      candidate,
      src,
      category,
      itemId,
    ...
    return {
      ok: applied,
      reason: applied ? 'revealed' : 'already_revealed',
      src: String(src).substring(0, 180),
      itemId: itemId || null,
      nodeId: getDiagNodeId(candidate),
    };
  }

  function forceRevealRehydrate(reason, options) {
    const actionReason = reason || 'host_force_reveal';
    const forceWebButton = !!(options && options.forceWebButton);
    if (forceWebButton) {
    ...
    for (let i = 0; i < candidates.length; i += 1) {
      const candidate = candidates[i];
      if (!candidate || candidate.nodeType !== 1 || !candidate.isConnected) continue;
      const src = candidate.dataset && candidate.dataset.mwSrc ? candidate.dataset.mwSrc : '';
      if (!src) continue;
      const category = candidate.dataset && candidate.dataset.mwCategory ? candidate.dataset.mwCategory : 'flagged';
      const itemId = candidate.dataset && candidate.dataset.mwItemId ? candidate.dataset.mwItemId : '';
      if (ensureRevealOverlayForBlurSignal(candidate, src, category, itemId, actionReason)) {
        ensured += 1;
      }
    }
    return {
      ...
      forceWebButton: !!window.__MW_NATIVE_REVEAL_FALLBACK__,
      reason: actionReason,
    };
  }
  ...
  function createRevealOverlay(element, src, category, itemId, allowShortsReresolve) {
    ...
    const forceWebRevealButton = window.__MW_NATIVE_REVEAL_FALLBACK__ === true;
    const useNativeRevealOverlay = CONFIG.nativeRevealOverlay === true && !forceWebRevealButton;
    let btn = null;
    if (!useNativeRevealOverlay) {
      ...
      btn.addEventListener('click', function(e) {
        ...
        const revealed = revealBlurredElement(
          element,
          src,
          category,
          itemId,
      ...
      console.log(
        '[DIAG][REVEAL_UI] native_overlay_mode',
        'overlayId=' + overlayId,
        'fallbackWebButton=' + forceWebRevealButton
      );
    }
    if (shortsMode) {
  ...
  function sendModerationRequest(items) {
    ...
    const requestNavId = getCurrentNavId();
    if (isShortsModeActive()) {
      console.log(
  ...
  function handleRequestTimeout(requestId) {
    ...
    diagScanBatchStartAtByRequestId.delete(requestId);
    const timeoutEpochMismatch = Number.isFinite(pendingRequest.pageEpoch) && pendingRequest.pageEpoch !== state.pageEpoch;
    if (timeoutEpochMismatch && !navIdsMatch(pendingRequest.navId, getCurrentNavId())) {
    ...
    if (timeoutEpochMismatch && navIdsMatch(pendingRequest.navId, getCurrentNavId())) {
      ...
      diagLogEpochBypass(
        ...
        'timeout_epoch_mismatch_nav_sync',
        pendingRequest.pageEpoch,
        state.pageEpoch,
    ...
    const forceShieldOnShortsTimeout = (
      FORCE_SHIELD_ON_SHORTS &&
      isShortsModeActive() &&
      pendingRequest.items.some(item => isKnownShortsHighRisk(item.src))
    );
    if (CONFIG.debug) {
      console.log(
        ...
        'shortsForce=' + forceShieldOnShortsTimeout
      );
    }
    if (timeoutPolicy.shouldBlur || forceShieldOnShortsTimeout) {
      console.log(
        '[MW] ' + (forceShieldOnShortsTimeout ? 'SHORTS-GUARD' : 'FAIL-CLOSED') + ': Applying blur to timed-out items'
      );
      pendingRequest.items.forEach(item => {
        ...
        if (element && element.isConnected) {
          applyBlur(
            element,
            item.src,
            forceShieldOnShortsTimeout ? 'shorts_timeout_guard' : 'timeout',
            CONFIG.blurStrength,
            item.itemId
          );
        }
  ...
  function handleModerationResult(message) {
    try {
      ...
      const pendingRequest = requestId ? state.pendingRequests.get(requestId) : null;
      const requestNavId = normalizeNavId((pendingRequest && pendingRequest.navId) || message.navId);
      const currentNavId = getCurrentNavId();
      const epochSyncAllowed = (
        resultEpoch !== null &&
        resultEpoch < state.pageEpoch &&
        navIdsMatch(requestNavId, currentNavId)
      );
      ...
      if (resultEpoch !== null && resultEpoch !== state.pageEpoch && (relaxedYouTubeEpochMode || epochSyncAllowed)) {
        ...
        diagLogEpochBypass(
          'epoch_bypass_allowed',
          epochSyncAllowed ? 'result_epoch_mismatch_nav_sync' : 'result_epoch_mismatch_youtube_relaxed',
          resultEpoch,
          state.pageEpoch,
          window.location.href
        );
      ...
      if (stickyShortsMode && resultEpoch !== null && resultEpoch < state.pageEpoch && !epochSyncAllowed) {
        console.warn(
      ...
      const shortsForceShieldEnabled = FORCE_SHIELD_ON_SHORTS && isShortsModeActive();
      ...
      if (forceShieldOnShorts && !state.revealed.has(src)) {
        finalBlur = true;
        decisionReason = (decisionReason ? decisionReason + '/' : '') + 'shorts_distraction_first';
      }
      ...
      if (!finalBlur && !aiShouldBlur && isShortsModeActive() && !forceShieldOnShorts) {
        try {
  ...
  function resolveYouTubePreferredBlurTarget(element, sourceType) {
    if (!element || element.nodeType !== 1) return element;
    if (!isYouTube()) return element;

    if (isShortsModeActive()) {
      if (typeof element.closest === 'function') {
        const shortsContainer = element.closest('.short-video-container');
        if (shortsContainer && shortsContainer.isConnected) {
          return shortsContainer;
        }
      }
      if (typeof element.querySelector === 'function') {
        const shortsDescendant = element.querySelector('.short-video-container');
        if (shortsDescendant && shortsDescendant.isConnected) {
          return shortsDescendant;
        }
      }
      return element;
    }

    const shouldScopeToThumbnail =
      sourceType === 'img' ||
      sourceType === 'video-poster' ||
      sourceType === 'bg-image';
    if (!shouldScopeToThumbnail) return element;

    if (typeof element.closest === 'function') {
      const closestThumbnail = element.closest('ytd-thumbnail, ytm-thumbnail-wrapper');
      if (closestThumbnail && closestThumbnail.isConnected) {
        return closestThumbnail;
      }
    }
    if (typeof element.querySelector === 'function') {
      const nestedThumbnail = element.querySelector('ytd-thumbnail, ytm-thumbnail-wrapper');
      if (nestedThumbnail && nestedThumbnail.isConnected) {
  ...
  function queueForScan(src, element, sourceType) {
    ...
    const preferredTarget = resolveYouTubePreferredBlurTarget(element, sourceType);
    if (preferredTarget && preferredTarget.nodeType === 1) {
      element = preferredTarget;
    }
    ...
    // Keep Shorts FORCE_SHIELD scoped to already-known high-risk items.
    // Unknown/new items start with soft blur until moderation returns.
    if (FORCE_SHIELD_ON_SHORTS && isShortsModeActive() && isKnownShortsHighRisk(url)) {
      clearSafeResolved(url);
      applyBlur(element, url, 'shorts_guard', CONFIG.blurStrength, itemId);
    } else {
  ...
  const YOUTUBE_SELECTORS = [
    'ytd-thumbnail',
    'ytd-thumbnail img',
    'ytm-thumbnail-wrapper',
    'ytm-thumbnail-wrapper img',
    '#thumbnail',
    '#thumbnail img',
    '.yt-core-image',
    'yt-img-shadow',
      ...
      for (const mutation of mutations) {
        ...
        mutation.addedNodes.forEach(node => {
          ...
          if (isYouTube() && isRelevantShortsContainerNode(node)) {
            ...
            startShortsReelTransitionObserver('container_added');
            forceRearmShortsShield('container_added', node);
          }
          if (shortsAttrMode) {
        ...
        if (mutation.type === 'attributes') {
          ...
          if (
            ...
            startShortsReelTransitionObserver('container_attr:' + attr);
            forceRearmShortsShield('container_attr:' + attr, target);
          }
  ...
  function setupYouTubeScrollHandler() {
    ...
    timerState.youtubeScrollHandler = () => {
      ...
      if (scrollDelta > 200) {
        ...
        if (FORCE_SHIELD_ON_SHORTS && isShortsModeActive() && hasKnownShortsHighRisk()) {
          forceRearmShortsShield('youtube_scroll', getActiveShortsPlayerContainer());
        }
  ...
  function countActiveTimerHandles() {
    ...
    if (timerState.shortsReelTransitionObserver) count++;
  ...
  if (!document.getElementById('mw-moderation-styles')) {
    ...
    style.textContent = \`
      ...
      ytd-thumbnail, ytm-thumbnail-wrapper, .short-video-container {
  ...
  window.__MW_NATIVE_REVEAL__ = function(reason) {
    try {
      return JSON.stringify(runNativeRevealRequest(reason || 'native_overlay'));
    } catch (e) {
      return JSON.stringify({ ok: false, reason: String(e || 'native_reveal_error') });
    }
  };
  window.__MW_FORCE_REVEAL_REHYDRATE__ = function(reason, options) {
    try {
      return JSON.stringify(forceRevealRehydrate(reason || 'host_force_rehydrate', options || null));
    } catch (e) {
      return JSON.stringify({ ok: false, reason: String(e || 'force_rehydrate_error') });
    }
  };
  ...
  const checkUrlChange = (triggerReason) => {
    if (window.location.href !== lastUrl) {
      ...
      clearAllRevealOverlays('url_change', true);
      clearAllShortsHighRisk('url_change');
      ...
      if (nextIsShorts) {
        ...
        startShortsReelTransitionObserver('url_change_shorts');
        forceRearmShortsShield('url_change_shorts', getActiveShortsPlayerContainer());
      } else {
        stopAdaptiveShortsOverlayWatch('url_change_non_shorts');
        stopShortsReelTransitionObserver('url_change_non_shorts');
      }
  ...
  function stopManagedTimers(reason) {
    ...
    stopShortsReelTransitionObserver('stopManagedTimers:' + (reason || 'unknown'));
  ...
  function startManagedTimers(reason) {
    ...
    startShortsReelTransitionObserver('startManagedTimers:' + (reason || 'unknown'));
  ...
  function teardownManagedScheduling(reason) {
    ...
    stopShortsReelTransitionObserver('teardown:' + (reason || 'unknown'));
  ...
  window.__MW_DEBUG__ = {
    ...
    nativeReveal: (reason) => runNativeRevealRequest(reason || 'debug_native_reveal'),
