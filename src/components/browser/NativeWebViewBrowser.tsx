...
const getYouTubeVideoIdFromMediaSrc = (value?: string): string => {
  const src = typeof value === 'string' ? value.trim() : '';
  if (!src) return 'none';
  const shortsMatch = src.match(/^shorts:\/\/video\/([^#/?]+)/i);
  if (shortsMatch && shortsMatch[1]) return shortsMatch[1];
  const ytImgMatch = src.match(/\/vi\/([^/?#]+)\//i);
...
const SHORTS_LEGACY_FALLBACK_MAX_PROBE_MS = 12000;
const SHORTS_LEGACY_FALLBACK_MAX_POLLS = 10;
const SHORTS_REVEAL_UI_MAX_FAILURE_RETRIES = 3;
const SHORTS_REVEAL_UI_RETRY_COOLDOWN_MS = 2200;
const SHORTS_REVEAL_UI_HARD_FALLBACK_LOCK_MS = 3800;
const SHORTS_TRANSPORT_UNKNOWN_FAIL_OPEN_WINDOW_MS = 5000;
...
const isMismatchToleratedSourceType = (sourceType: string): boolean => {
  void sourceType;
  // Shorts MVP safety: never mark ID mismatches as tolerated-safe.
  return false;
};
...
export const NativeWebViewBrowser = () => {
  ...
  const blurReadyNavMatchRef = useRef<{
    navId: number;
    pageEpoch: number;
    url: string;
    reason: string;
  ...
  const blurReadyPerfAlarmRef = useRef<{ navId: number; pageEpoch: number; at: number }>({
    navId: 0,
    pageEpoch: 0,
    at: 0,
  });
  const blurReadySettleWindowRef = useRef<{
    active: boolean;
    navId: number;
    untilMs: number;
    reason: string;
  ...
  const blurReadySettleWindowTimerRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const lastShortsRevealTransportUnknownRef = useRef<{
    at: number;
    sovereignId: string;
    reason: string;
  }>({
  ...
  const shortsRevealUiFailureRef = useRef<Map<string, {
    attempts: number;
    lastAt: number;
    lastCapAt: number;
  }>>(new Map());
  ...
  const lastShortsReplaceStateReadyRef = useRef<{ sovereignId: string; at: number }>({
    sovereignId: '',
    at: 0,
  });
  ...
  const STABLE_INJECTION_URL_CHANGE_DEBOUNCE_MS = 150;
  const BLUR_READY_PING_SUPPRESS_MS = 400;
  const BLUR_READY_REPLACESTATE_SUPPRESS_MS = 1000;
  const OVERLAY_LIFT_HANDSHAKE_MS = 50;
  const SHORTS_ATOMIC_TIMEOUT_MS = 1600;
  const SHORTS_MATCHING_RETRY_DELAY_MS = 500;
  const SHORTS_MISMATCH_HOLD_MS = 900;
  ...
  useEffect(() => {
    console.log('[DIAG][SHORTS_SYNC]', 'hostTag=' + SHORTS_MVP_SYNC_TAG);
  }, []);
  const clearBlurReadyNavMatchState = useCallback((reason: string) => {
    const previous = blurReadyNavMatchRef.current;
    blurReadyNavMatchRef.current = {
      navId: 0,
      pageEpoch: 0,
      url: '',
      reason: reason || 'reset',
    ...
    debugLog(
      ...
      'action=nav_match_reset',
      'reason=' + (reason || 'reset'),
      'prevNavId=' + previous.navId,
      'prevPageEpoch=' + previous.pageEpoch,
      'prevAgeMs=' + Math.max(0, Date.now() - previous.at),
    );
  }, [debugLog]);
  ...
  const armShortsLegacyFallbackProbe = useCallback((reason: string, durationMs: number) => {
    ...
    const hasActiveProbe = state.untilMs > now;
    const remainingMs = hasActiveProbe ? Math.max(0, state.untilMs - now) : 0;
    const reasonChanged = state.reason !== reason;
    if (hasActiveProbe && !reasonChanged && nextUntil <= state.untilMs) {
      return;
    }
    // Coalesce churn: if a probe is already active for long enough, keep it active
    // without forcing a poll-effect restart on every reason change.
    if (hasActiveProbe && reasonChanged && remainingMs >= 700 && nextUntil <= state.untilMs) {
      state.reason = reason;
      console.log(
        '[MW-Host][ShortsFallback] probe_keepalive',
        'reason=' + reason,
  ...
  const resetShortsOverlayCoordinator = useCallback((reason: string) => {
    ...
    shortsRevealUiFailureRef.current.clear();
  ...
  const armShortsOverlayAtomicTimeout = useCallback((sovereignId: string, startedAtMs?: number) => {
    ...
    const delayMs = Math.max(0, SHORTS_ATOMIC_TIMEOUT_MS - elapsedMs);
    shortsOverlayTimeoutRef.current = setTimeout(() => {
      ...
      console.log(
        ...
        'hasModerationResult=' + pending.hasModerationResult,
        'result=' + (pending.result || 'none'),
        'timeoutMs=' + SHORTS_ATOMIC_TIMEOUT_MS,
        'elapsedMs=' + (Date.now() - pending.startedAt),
      );
      queueCurrentBlurState('shorts_atomic_timeout_release');
    }, delayMs);
  }, [SHORTS_ATOMIC_TIMEOUT_MS, clearShortsOverlayAtomicTimeout, hasEffectiveShortsBlurSignal, queueCurrentBlurState]);
  ...
  const markShortsOverlayJsState = useCallback((
    ...
    const isUserRevealSuccessSignal = signalReason === 'reveal_button_success';
    if (isUserRevealSuccessSignal) {
      const lock = getShortsHardRevealLockForSovereign(sovereignId)
        || setShortsHardRevealLockForSovereign(sovereignId, 'reveal_button_success');
      if (lock) {
        hydrateShortsPendingStateFromHardRevealLock(
          sovereignId,
          'mark_js_state:reveal_button_success',
        );
        console.log(
          '[DIAG][REVEAL_LOCK]',
          'action=set_from_js_state',
          'reason=' + signalReason,
          'videoId=' + lock.videoId,
          'sovereignId=' + sovereignId,
        );
        queueCurrentBlurState('shorts_atomic_reveal_button_success_hard_lock');
        return;
      }
    }
    const activeUrl = currentUrlRef.current || '';
    ...
    const shouldPreserveBlurSignal = (
      ...
      jsRevealApplied &&
      !jsBlurApplied &&
      signalReason !== 'remove_blur' &&
      signalReason !== 'reveal_button_success'
    );
  ...
  const markShortsOverlayModerationResult = useCallback((
    ...
    pending.mismatchHoldUntil = 0;
    const shouldConvertMismatchHoldToPending = (
      effectiveResult === 'BLUR' &&
      effectiveDecisionReason === 'id_mismatch_tolerated_hold' &&
      !hardRevealLock
    );
    if (shouldConvertMismatchHoldToPending) {
      pending.hasModerationResult = false;
      pending.result = null;
      pending.hasBlurSignal = true;
      ...
      console.log(
        '[DIAG][SHORTS_ATOMIC]',
        'action=id_mismatch_pending_window',
        'reason=' + effectiveDecisionReason,
        'sovereignId=' + sovereignId,
        'holdMs=' + SHORTS_MISMATCH_HOLD_MS,
      );
      armShortsLegacyFallbackProbe('id_mismatch_pending_window', 1200);
      armShortsOverlayAtomicTimeout(sovereignId, pending.startedAt);
      queueCurrentBlurState('shorts_atomic_id_mismatch_pending_window');
      return;
    }
    console.log(
      '[DIAG][SHORTS_ATOMIC]',
    ...
    const shouldHoldClearForMismatchRetry = false;
  ...
  const {
    ...
    onLoadStart: (url) => {
      ...
      clearStableInjectionTimer('load_start_hard_kill');
      stableInjectionPendingRef.current = null;
      const killedLoadStartRequests = pendingRequestsRef.current.size;
      if (killedLoadStartRequests > 0) {
        pendingRequestsRef.current.clear();
        console.warn(
          '[DIAG][SHORTS_ATOMIC]',
          'action=hard_kill_pending_requests',
          'reason=load_start',
          'killed=' + killedLoadStartRequests,
          'navId=' + activeNavIdRef.current,
          'generation=' + moderationNavGenerationRef.current,
          'url=' + (url || 'unknown'),
        );
      }
      armBlurReadySettleWindow('load_start', url);
    ...
    onLoadError: (url, error) => {
      ...
      clearBlurReadySettleWindowState('load_error');
    ...
    onUrlChange: (url) => {
      ...
      moderationNavGenerationRef.current += 1;
      clearStableInjectionTimer('url_change_hard_kill');
      stableInjectionPendingRef.current = null;
      const killedUrlChangeRequests = pendingRequestsRef.current.size;
      if (killedUrlChangeRequests > 0) {
        pendingRequestsRef.current.clear();
        console.warn(
          '[DIAG][SHORTS_ATOMIC]',
          'action=hard_kill_pending_requests',
          'reason=url_change',
          'killed=' + killedUrlChangeRequests,
          'navId=' + activeNavIdRef.current,
          'generation=' + moderationNavGenerationRef.current,
          'url=' + (url || 'unknown'),
        );
      ...
      if (isNative && webViewState.isOpen && executeScript) {
        ...
        const syncNavId = activeNavIdRef.current || 0;
        const syncPageEpoch = webViewPageEpochRef.current || 0;
        const syncActiveInstanceId = (
          typeof webViewActiveInstanceIdRef.current === 'number' &&
          Number.isFinite(webViewActiveInstanceIdRef.current)
        ) ? webViewActiveInstanceIdRef.current : null;
        void executeScript(`
          (function() {
            try {
              window.__MW_ACTIVE_INSTANCE_ID__ = ${syncActiveInstanceId === null ? 'null' : String(syncActiveInstanceId)};
              window.__MW_HOST_NAV_ID__ = ${syncNavId};
              window.__MW_HOST_PAGE_EPOCH__ = ${syncPageEpoch};
              if (
                window.__MW_ACTIVE_CONTEXT_API__ &&
                typeof window.__MW_ACTIVE_CONTEXT_API__.applyHostContextSync === 'function'
              ) {
                window.__MW_ACTIVE_CONTEXT_API__.applyHostContextSync('${syncReason}');
              }
              return 'OK';
            } catch (e) {
              return 'ERR';
        ...
        `).then((syncResult) => {
          if (String(syncResult || '') === 'ERR') {
            console.warn(
              '[DIAG][SHORTS_ATOMIC]',
              'action=fast_context_sync_failed',
              'reason=url_change',
              'navId=' + syncNavId,
              'pageEpoch=' + syncPageEpoch,
              'url=' + (url || 'unknown'),
            );
          }
        }).catch((error) => {
          console.warn(
            '[DIAG][SHORTS_ATOMIC]',
            'action=fast_context_sync_error',
            'reason=url_change',
            'navId=' + syncNavId,
            'pageEpoch=' + syncPageEpoch,
            'message=' + (error instanceof Error ? error.message : String(error)),
      ...
      injectionInFlightRef.current = false;
      blurReadyRef.current = false;
      clearBlurReadyNavMatchState('url_change');
      ...
      if (shouldSkipHardResetForProfileReveal) {
        ...
        void postBlurCommand('DISABLE_BLUR', 'url_change_hard_reset_instant').then((delivered) => {
          ...
          console.log(
            '[DIAG][OVERLAY_HOST_STATE]',
            'action=instant_reset_command_skipped',
            'reason=url_change_hard_reset_instant',
            'navId=' + activeNavIdRef.current,
            'url=' + (url || 'unknown'),
          );
        });
      ...
      armBlurReadySettleWindow('url_change', url);
    ...
    onClose: () => {
      ...
      clearBlurReadySettleWindowState('webview_closed');
      clearLoadEndInjectTimer();
    ...
    onActiveInstanceIdChange: (activeInstanceId) => {
      ...
      clearBlurReadySettleWindowState('active_instance_change');
      blurReadyRef.current = false;
      clearBlurReadyNavMatchState('active_instance_change');
  ...
  const ensureShortsRevealUi = useCallback(async (
    ...
    const ensureResult = await executeScript(`
      (function() {
        try {
          ...
          forceButtonLayer();
          var revealBtnPresent = !!document.querySelector('.mw-reveal-btn, .mw-reveal-button');
          return ensured || revealBtnPresent;
        } catch (e) {
          return false;
        }
      })();
    ...
    const ensureTransportUnknown = (
      (ensureResult === null || typeof ensureResult === 'undefined') &&
      (ensureRetryResultLabel === 'null' || ensureRetryResultLabel === 'undefined')
    );
    if (ensureTransportUnknown) {
      lastShortsRevealTransportUnknownRef.current = {
        at: Date.now(),
        sovereignId: sovereignId || 'none',
        reason: reason || 'id_mismatch_fallback',
      };
      console.warn(
        '[DIAG][SHORTS_ATOMIC]',
        'action=reveal_ui_transport_unknown',
        'reason=' + reason,
        'requestId=' + requestId,
        'sovereignId=' + sovereignId,
        'videoId=' + videoId,
      ...
      // Explicitly request a reveal UI sync over the host->webview message channel.
      // executeScript can return null during iOS Shorts churn even when JS is alive.
      if (sovereignId && sovereignId !== 'none') {
        syncShortsRevealRequestToWebViewRef.current(
          sovereignId,
          'transport_unknown:' + (reason || 'id_mismatch_fallback'),
        );
      }
      void requestShortsReentryRefresh('ensure_reveal_ui_transport_unknown', currentUrlRef.current || '', true);
      armShortsLegacyFallbackProbe('ensure_reveal_ui_transport_unknown:' + reason, 3200);
      queueCurrentBlurState('shorts_atomic_reveal_ui_transport_unknown');
      return;
    }
    const failureKey = [sovereignId || 'none', reason || 'unknown'].join('|');
    const now = Date.now();
    const failureState = shortsRevealUiFailureRef.current.get(failureKey) || {
      attempts: 0,
      lastAt: 0,
      lastCapAt: 0,
    };
    const withinCooldown = (now - failureState.lastAt) <= SHORTS_REVEAL_UI_RETRY_COOLDOWN_MS;
    if (ensureResultBool) {
      shortsRevealUiFailureRef.current.delete(failureKey);
      const transportUnknown = lastShortsRevealTransportUnknownRef.current;
      if (transportUnknown.sovereignId === (sovereignId || 'none')) {
        lastShortsRevealTransportUnknownRef.current = {
          at: 0,
          sovereignId: 'none',
          reason: 'cleared_after_ensure_success',
        };
      }
    } else {
      const nextAttempts = withinCooldown ? (failureState.attempts + 1) : 1;
      shortsRevealUiFailureRef.current.set(failureKey, {
        attempts: nextAttempts,
        lastAt: now,
        lastCapAt: failureState.lastCapAt || 0,
      });
    }
    const updatedFailureState = shortsRevealUiFailureRef.current.get(failureKey);
    const overRetryCap = !!(
      !ensureResultBool &&
      updatedFailureState &&
      updatedFailureState.attempts > SHORTS_REVEAL_UI_MAX_FAILURE_RETRIES &&
      (now - updatedFailureState.lastAt) <= SHORTS_REVEAL_UI_RETRY_COOLDOWN_MS
    );
    console.log(
      '[DIAG][SHORTS_ATOMIC]',
      'action=id_mismatch_reveal_ui',
      ...
      'raw=' + ensureResultLabel,
      'retry=' + ensureRetryResultLabel,
      'attempts=' + String(updatedFailureState?.attempts || 0),
      'retryCap=' + (overRetryCap ? 'hit' : 'ok'),
    );
    if (!ensureResultBool) {
      if (sovereignId && sovereignId !== 'none') {
        syncShortsRevealRequestToWebViewRef.current(
          sovereignId,
          'ensure_failed_retry:' + (reason || 'id_mismatch_fallback'),
        );
      }
      if (overRetryCap) {
        const shouldEmitCapAction = (now - (updatedFailureState?.lastCapAt || 0)) > 900;
        if (updatedFailureState && shouldEmitCapAction) {
          shortsRevealUiFailureRef.current.set(failureKey, {
            attempts: updatedFailureState.attempts,
            lastAt: updatedFailureState.lastAt,
            lastCapAt: now,
          });
        }
        console.warn(
          '[DIAG][SHORTS_ATOMIC]',
          'action=reveal_ui_retry_cap_hit',
          'reason=' + reason,
          'requestId=' + requestId,
          'sovereignId=' + sovereignId,
          'videoId=' + videoId,
        ...
        if (shouldEmitCapAction) {
          void executeScript(`
          (function() {
            try {
              if (typeof window.__MW_START_REVEAL_WATCH__ === 'function') {
                try {
                  window.__MW_START_REVEAL_WATCH__('host_retry_cap:${safeReason}', {
                    key: '${safeSovereignId}',
                    force: true,
                    videoId: '${safeVideoId}'
                  });
              ...
              if (typeof window.__MW_APPLY_REVEAL_UI_LOCK__ === 'function') {
                try {
                  window.__MW_APPLY_REVEAL_UI_LOCK__('host_retry_cap:${safeReason}', ${SHORTS_REVEAL_UI_HARD_FALLBACK_LOCK_MS}, '${safeVideoId}');
                } catch (lockErr) {}
              }
              if (typeof window.__MW_ENSURE_REVEAL_UI__ === 'function') {
                try { window.__MW_ENSURE_REVEAL_UI__('host_retry_cap:${safeReason}'); } catch (ensureErr) {}
              }
              return true;
            } catch (e) {
              return false;
          ...
          `).catch(() => undefined);
          armShortsLegacyFallbackProbe('ensure_reveal_ui_retry_cap:' + reason, 900);
          queueCurrentBlurState('shorts_atomic_reveal_ui_retry_cap');
        }
      } else {
  ...
  const postBlurCommand = useCallback(async (
    command: 'ENABLE_BLUR' | 'DISABLE_BLUR' | 'PING',
    reason: string,
  ): Promise<boolean> => {
    if (!ENABLE_DOM_BLUR) return false;
    if (!isNative || !webViewState.isOpen || !executeScript) return false;
    const commandLiteral = command;
    const reasonLiteral = escapeForJs(reason || 'host');
    const result = await executeScript(`
      (function() {
        try {
          window.postMessage({
  ...
  const clearBlurReadySettleWindowState = useCallback((reason: string) => {
    const settleState = blurReadySettleWindowRef.current;
    const wasActive = settleState.active;
    const elapsedMs = settleState.startedAt > 0
      ? Math.max(0, Date.now() - settleState.startedAt)
      : 0;
    if (blurReadySettleWindowTimerRef.current) {
      clearTimeout(blurReadySettleWindowTimerRef.current);
      blurReadySettleWindowTimerRef.current = null;
    }
    settleState.active = false;
    settleState.navId = 0;
    settleState.untilMs = 0;
    settleState.reason = reason || 'cleared';
    settleState.startedAt = 0;
    if (!wasActive) return;
    console.log(
      '[DIAG][SETTLE_WINDOW_HOST]',
      'action=clear',
      'reason=' + (reason || 'unknown'),
      'elapsedMs=' + elapsedMs,
    );
  ...
  const armBlurReadySettleWindow = useCallback((reason: string, urlHint?: string) => {
    if (!ENABLE_DOM_BLUR) return;
    const targetUrl = urlHint || currentUrlRef.current || webViewState.currentUrl || '';
    if (getUrlFamily(targetUrl) !== 'youtube_shorts') {
      const hadActiveSettleWindow = blurReadySettleWindowRef.current.active;
      clearBlurReadySettleWindowState('skip_non_shorts:' + reason);
      if (hadActiveSettleWindow) {
        const restoreEnabled = blurStateRef.current.enabled;
        const restoreReason = 'settle_window_skip_non_shorts_restore:' + (reason || 'url_change');
        const restoreCommand = restoreEnabled ? 'ENABLE_BLUR' : 'DISABLE_BLUR';
        void postBlurCommand(restoreCommand, restoreReason);
        queueCurrentBlurState(restoreReason);
      }
      return;
    }

    clearBlurReadySettleWindowState('rearm:' + reason);

    const now = Date.now();
    const navId = activeNavIdRef.current || 0;
    blurReadySettleWindowRef.current = {
      active: true,
      navId,
      untilMs: now + BLUR_READY_SETTLE_WINDOW_MS,
    ...
    console.log(
      ...
      'reason=' + (reason || 'url_change'),
      'navId=' + navId,
      'windowMs=' + BLUR_READY_SETTLE_WINDOW_MS,
      'url=' + (targetUrl || 'unknown'),
    );
    void postBlurCommand('ENABLE_BLUR', 'settle_window_force:' + (reason || 'url_change')).then((delivered) => {
      if (delivered) return;
      console.log(
        '[DIAG][SETTLE_WINDOW_HOST]',
        'action=force_enable_skipped',
        'reason=' + (reason || 'url_change'),
        'navId=' + navId,
      );
    });

    blurReadySettleWindowTimerRef.current = setTimeout(() => {
      const settleState = blurReadySettleWindowRef.current;
      if (!settleState.active || settleState.navId !== navId) return;
      const elapsedMs = settleState.startedAt > 0
        ? Math.max(0, Date.now() - settleState.startedAt)
        : 0;
      console.log(
        '[DIAG][SETTLE_WINDOW_HOST]',
        'action=timeout_release',
        'reason=' + (settleState.reason || reason || 'url_change'),
        'navId=' + navId,
        'elapsedMs=' + elapsedMs,
      );
      clearBlurReadySettleWindowState('timeout_release:' + (reason || 'url_change'));
      const restoreEnabled = blurStateRef.current.enabled;
      const restoreReason = 'settle_window_timeout_restore:' + (reason || 'url_change');
      const restoreCommand = restoreEnabled ? 'ENABLE_BLUR' : 'DISABLE_BLUR';
      void postBlurCommand(restoreCommand, restoreReason);
      queueCurrentBlurState(restoreReason);
  ...
  const requestBlurHandshake = useCallback(async (source: string) => {
    await postBlurCommand('PING', source);
  }, [postBlurCommand]);
  ...
  const getStableInjectionDebounceMs = useCallback((urlHint?: string, reasonHint?: string) => {
    const normalizedReason = String(reasonHint || '');
    if (
      normalizedReason === 'onUrlChange' ||
      normalizedReason.startsWith('context_shift:onUrlChange')
    ) {
      return STABLE_INJECTION_URL_CHANGE_DEBOUNCE_MS;
    }
    const resolvedUrl = urlHint || webViewState.currentUrl || currentUrlRef.current || '';
    ...
    STABLE_INJECTION_SHORTS_DEBOUNCE_MS,
    STABLE_INJECTION_DEBOUNCE_MS,
    STABLE_INJECTION_URL_CHANGE_DEBOUNCE_MS,
    webViewState.currentUrl,
  ]);
  ...
  const scheduleStableInjection = useCallback((reason: string, urlHint?: string) => {
    ...
    const debounceMs = getStableInjectionDebounceMs(targetUrl, reason);
    stableInjectionTimerRef.current = setTimeout(() => {
      ...
      if (!hasActiveInstance) {
        console.log(
          '[DIAG][INJECT_STABLE]',
          'action=skip_no_active_instance',
          'reason=' + pendingState.reasons.join('+'),
          'key=' + pendingState.key,
          'navId=' + pendingState.navId,
          'url=' + pendingState.url,
        );
        return;
      }
      ...
      const activeDebounceMs = getStableInjectionDebounceMs(activeUrl, pendingState.reasons[0] || reason);
  ...
  const shouldHoldShortsOverlaySync = useCallback(() => {
    ...
    const timedOut = (Date.now() - pending.startedAt) >= SHORTS_ATOMIC_TIMEOUT_MS;
    return !timedOut;
  }, [SHORTS_ATOMIC_TIMEOUT_MS, webViewState.currentUrl, getSovereignIdForContext, hasEffectiveShortsBlurSignal]);
    ...
    if (!holdShortsOverlaySync) {
      if (activeUrlFamily === 'youtube_shorts') {
        ...
        const navMatchConfirmed = (
          navMatchState.at > 0 &&
          navMatchState.navId === activeNavId &&
          navMatchState.pageEpoch === activeEpoch
        );
        const settleState = blurReadySettleWindowRef.current;
        const hasActiveSettleWindow = settleState.active && settleState.navId === activeNavId;
        const navMatchAgeMs = navMatchConfirmed ? Math.max(0, now - navMatchState.at) : -1;
        ...
        const timedOutWithoutResult = !!pendingShorts &&
          ...
          !pendingShorts.hasModerationResult &&
          pendingShorts.mismatchHoldUntil <= now &&
          (now - pendingShorts.startedAt) >= SHORTS_ATOMIC_TIMEOUT_MS;
        const mismatchHoldActive = !!pendingShorts && pendingShorts.mismatchHoldUntil > now;
        if (mismatchHoldActive && pendingShorts && (now - pendingShorts.holdUntilReadyLastLogAt) >= 400) {
          pendingShorts.holdUntilReadyLastLogAt = now;
          console.log(
            '[DIAG][SHORTS_ATOMIC]',
            'action=hold_pending_mismatch_window',
            'sovereignId=' + pendingShorts.sovereignId,
            'remainingMs=' + Math.max(0, pendingShorts.mismatchHoldUntil - now),
          );
        }
        if (timedOutWithoutResult && !pendingShorts.fallbackRevealEnsured) {
          if (holdUntilReady) {
            pendingShorts.holdUntilReady = true;
            if (pendingShorts.holdUntilReadySince <= 0) {
              pendingShorts.holdUntilReadySince = now;
            }
            if ((now - pendingShorts.holdUntilReadyLastLogAt) >= 400) {
              pendingShorts.holdUntilReadyLastLogAt = now;
              console.log(
                '[DIAG][SHORTS_ATOMIC]',
                'action=hold_until_ready',
                'holdReason=' + (!navMatchConfirmed ? 'await_nav_match' : 'await_settle_window'),
                'sovereignId=' + pendingShorts.sovereignId,
                'navId=' + activeNavId,
                'pageEpoch=' + activeEpoch,
                'readyNavId=' + (navMatchState.navId || 0),
                'readyPageEpoch=' + (navMatchState.pageEpoch || 0),
                'readyAgeMs=' + (navMatchAgeMs >= 0 ? navMatchAgeMs : 'none'),
                'settleActive=' + hasActiveSettleWindow,
                'settleRemainingMs=' + settleRemainingMs,
                'elapsedMs=' + (now - pendingShorts.startedAt),
              );
            }
          } else {
            if (pendingShorts.holdUntilReady) {
              console.log(
                '[DIAG][SHORTS_ATOMIC]',
                'action=release_hold_until_ready',
                'sovereignId=' + pendingShorts.sovereignId,
                'navId=' + activeNavId,
                'pageEpoch=' + activeEpoch,
                'holdMs=' + Math.max(0, now - pendingShorts.holdUntilReadySince),
                'readyAgeMs=' + (navMatchAgeMs >= 0 ? navMatchAgeMs : 'none'),
                'settleRemainingMs=' + settleRemainingMs,
                'settleActive=' + hasActiveSettleWindow,
              );
            }
            pendingShorts.holdUntilReady = false;
            pendingShorts.holdUntilReadySince = 0;
            ...
            const protectedBlurState = {
              enabled: false,
              reason: 'shorts_atomic_timeout_safe_failure_fail_open',
              timestamp: Date.now(),
            };
            ...
            void postBlurCommand('DISABLE_BLUR', 'shorts_atomic_timeout_safe_failure_fail_open').catch(() => undefined);
            void ensureShortsRevealUi(
  ...
  useEffect(() => {
    ...
    if (ENABLE_DOM_BLUR) {
      clearBlurReadySettleWindowState('settings_reinject');
      blurReadyRef.current = false;
      clearBlurReadyNavMatchState('settings_reinject');
  ...
  const processModerationRequest = useCallback(async (request: ModerationRequestMessage, nonce: string) => {
    ...
    const effectiveRequestSovereignId = requestSovereignId;
    ...
    const hardKillStaleRequest = (stage: string): boolean => {
      ...
      disarmFlashShield('moderation_hard_kill_stale', 'disarm hard-killed stale request');
      return true;
    };
    ...
    for (const item of items) {
      ...
      if (stickyShortsMode && enforceShortsAtomicVideoMatch && !srcMatchesCurrentVideo) {
        ...
        const mismatchCategory = 'shorts_mismatch_hold';
        results.push({
          itemId: item.itemId,
          src: item.src,
          shouldBlur: true,
    ...
    if (holdForMismatchedThumbnails || shouldHoldToleratedMismatchForRetry) {
    ...
    } else if (shouldHoldToleratedMismatchForRetry) {
      // Keep blur during tolerated mismatch windows to avoid clear/blur thrash.
      overlayDecision = true;
      decisionReason = 'id_mismatch_tolerated_hold';
    ...
    if (hardKillStaleRequest('post_decision_pre_apply')) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleIncomingMessage = async (rawPayload: unknown, source: 'capgo' | 'window') => {
      ...
      if (typedMessage.type === 'MW_JS_REVEAL_STATE') {
        ...
        const signalSrc = typeof typedMessage.src === 'string' ? typedMessage.src : '';
        const signalSrcVideoId = getYouTubeVideoIdFromMediaSrc(signalSrc);
        const messageVideoId = parseSovereignId(messageSovereignId).videoId || 'none';
        const activeVideoId = parseSovereignId(getSovereignIdForContext(activeUrl)).videoId || 'none';
        const targetVideoId = messageVideoId !== 'none' ? messageVideoId : activeVideoId;
        if (
          signalReason === 'clear_blur_overlay' &&
          signalSrcVideoId !== 'none' &&
          targetVideoId !== 'none' &&
          signalSrcVideoId !== targetVideoId
        ) {
          console.warn(
            '[DIAG][SHORTS_ATOMIC]',
            'action=ignore_clear_blur_src_video_mismatch',
            'sovereignId=' + messageSovereignId,
            'targetVideoId=' + targetVideoId,
            'signalSrcVideoId=' + signalSrcVideoId,
            'signalSrc=' + signalSrc.substring(0, 180),
          );
          queueCurrentBlurState('shorts_atomic_ignore_clear_blur_src_video_mismatch');
          armShortsLegacyFallbackProbe('clear_blur_src_video_mismatch_recover', 1200);
          return;
        }
      ...
      if (ENABLE_DOM_BLUR && isBlurOverlayReadyMessage(message)) {
        ...
        if (readySignalAgeMs !== null && readySignalAgeMs > 500) {
          ...
          const shouldLogPerfAlarm = (
            lastAlarm.navId !== activeNavId ||
            lastAlarm.pageEpoch !== activeEpoch ||
            (now - lastAlarm.at) >= 1200
          );
          if (shouldLogPerfAlarm) {
            blurReadyPerfAlarmRef.current = {
              navId: activeNavId,
              pageEpoch: activeEpoch,
              at: now,
            };
            console.warn(
              '[PERF_ALARM]',
              'metric=MW_BLUR_READY_LATENCY',
              'latencyMs=' + readySignalAgeMs,
              'thresholdMs=500',
              'reason=' + readyReason,
              'activeNavId=' + activeNavId,
              'messageHostNavId=' + (readyHostNavId ?? 'none'),
              'activePageEpoch=' + activeEpoch,
              'messagePageEpoch=' + (readyEpoch ?? 'none'),
              'url=' + readyUrl,
            );
          }
        }
        ...
        if (staleEpoch || (staleNav && !navMismatchGrace)) {
          ...
          console.warn(
            '[DIAG][SHORTS_ATOMIC]',
            'action=hard_kill_stale_blur_ready',
            'reason=' + readyReason,
            'hardDropReason=' + hardDropReason,
            'messageEpoch=' + (readyEpoch ?? 'none'),
            'activeEpoch=' + activeEpoch,
            'messageHostNavId=' + (readyHostNavId ?? 'none'),
            'activeNavId=' + activeNavId,
            'sovereignId=' + (readySovereignId || 'none'),
            'url=' + readyUrl,
          );
          if (activeUrlFamily === 'youtube_shorts') {
            clearBlurReadyNavMatchState('stale_blur_ready:' + hardDropReason + ':' + readyReason);
            void requestBlurHandshake('stale_blur_ready_drop:' + hardDropReason + ':' + readyReason)
              .catch(() => undefined);
          }
          return;
        }
        if (activeUrlFamily === 'youtube_shorts') {
          const lastNavMatch = blurReadyNavMatchRef.current;
          const hasNavMatchForActiveFrame = (
            lastNavMatch.navId === activeNavId &&
            lastNavMatch.pageEpoch === activeEpoch &&
            lastNavMatch.at > 0
          );
          if (!hasNavMatchForActiveFrame) {
            blurReadyNavMatchRef.current = {
              navId: activeNavId,
              pageEpoch: activeEpoch,
              url: readyUrl,
        ...
        if (settleNavExactMatch) {
          const settleElapsedMs = settleState.startedAt > 0
            ? Math.max(0, now - settleState.startedAt)
            : 0;
          clearBlurReadySettleWindowState('first_valid_nav_match:' + readyReason);
          console.log(
            '[DIAG][SETTLE_WINDOW_HOST]',
            'action=first_valid_nav_match',
            'reason=' + readyReason,
            'navId=' + activeNavId,
        ...
        if (activeUrlFamily === 'youtube_shorts' && isSettleHistoryReadyReason) {
          ...
          const settleAlreadyTrackingActiveNav = (
            ...
            liveSettleState.navId === activeNavId
          );
          if (!settleAlreadyTrackingActiveNav) {
            armBlurReadySettleWindow('url_change:' + readyReason, readyUrl);
            debugLog(
              '[DIAG][SETTLE_WINDOW_HOST]',
              'action=arm_from_history_ready',
              'reason=' + readyReason,
              'navId=' + activeNavId,
              'url=' + readyUrl,
            );
        ...
        if (activeUrlFamily === 'youtube_shorts' && readySovereignId) {
          ...
          const isPingOrReinjectReady = readyReason === 'ping' || readyReason === 'reinject';
          const suppressPingPongAfterReplaceState = (
            isPingOrReinjectReady &&
            lastReplaceStateReady.sovereignId === readySovereignId &&
            (now - lastReplaceStateReady.at) >= 0 &&
            (now - lastReplaceStateReady.at) <= BLUR_READY_REPLACESTATE_SUPPRESS_MS
          );
          if (suppressPingPongAfterReplaceState) {
            console.log(
              '[DIAG][BLUR_READY_GATE]',
              'action=drop_ping_pong_after_replace_state',
              'reason=' + readyReason,
              'sovereignId=' + readySovereignId,
              'lastReplaceStateAgeMs=' + (now - lastReplaceStateReady.at),
              'windowMs=' + BLUR_READY_REPLACESTATE_SUPPRESS_MS,
              'url=' + readyUrl,
            );
            return;
          }
          ...
          const sameSovereignReplaceState = (
            ...
            (now - lastShortsBlurSignal.at) < SHORTS_ATOMIC_TIMEOUT_MS
          );
  ...
  useEffect(() => {
    ...
    const shouldFailOpenShortsProbeTimeout = (): boolean => {
      const transportUnknown = lastShortsRevealTransportUnknownRef.current;
      if (!transportUnknown.at) return false;
      const ageMs = Math.max(0, Date.now() - transportUnknown.at);
      if (ageMs > SHORTS_TRANSPORT_UNKNOWN_FAIL_OPEN_WINDOW_MS) return false;
      const activeSovereignId = getSovereignIdForContext(activeUrl);
      const activeVideoId = parseSovereignId(activeSovereignId).videoId || 'none';
      const signalVideoId = parseSovereignId(transportUnknown.sovereignId).videoId || 'none';
      if (activeVideoId === 'none' || signalVideoId === 'none' || activeVideoId !== signalVideoId) {
        return false;
      }
      console.warn(
        '[DIAG][SHORTS_ATOMIC]',
        'action=fail_open_transport_unknown_gate_allow',
        'activeSovereignId=' + activeSovereignId,
        'signalSovereignId=' + transportUnknown.sovereignId,
        'signalReason=' + (transportUnknown.reason || 'none'),
        'signalAgeMs=' + ageMs,
        'windowMs=' + SHORTS_TRANSPORT_UNKNOWN_FAIL_OPEN_WINDOW_MS,
      );
      return true;
    };
    ...
    const applyShortsFailClosed = (reason: string) => {
      ...
      const activeVideoId = parseSovereignId(activeSovereignId).videoId || 'none';
      const pending = shortsOverlayPendingRef.current;
      const hasActiveClearDecision = !!(
        pending &&
        pending.sovereignId === activeSovereignId &&
        pending.hasModerationResult &&
        pending.result === 'CLEAR'
      );
      if (hasActiveClearDecision && lastProducerMode === 'unknown') {
        console.warn(
          '[DIAG][SHORTS_ATOMIC]',
          'action=skip_fail_closed_recent_clear',
          'reason=' + reason,
          'sovereignId=' + activeSovereignId,
          'videoId=' + activeVideoId,
          'producer=' + lastProducerMode,
      ...
      const hardRevealLock = getShortsHardRevealLockForSovereign(activeSovereignId);
      if (hardRevealLock) {
        hydrateShortsPendingStateFromHardRevealLock(activeSovereignId, 'fail_closed:' + (reason || 'unknown'));
        console.warn(
          '[DIAG][SHORTS_ATOMIC]',
          'action=skip_fail_closed_hard_reveal_lock',
          'reason=' + reason,
          'sovereignId=' + activeSovereignId,
          'videoId=' + hardRevealLock.videoId,
        );
        queueCurrentBlurState('shorts_fail_closed_skipped_hard_reveal_lock');
        return;
      }
      pendingForceRevealEnforcementRef.current = null;
      const protectedBlurState = {
      ...
      if (activeVideoId !== 'none' && executeScript) {
        const safeVideoId = escapeForJs(activeVideoId);
        const safeReason = escapeForJs(reason || 'unknown');
        const safeSovereignId = escapeForJs(activeSovereignId || 'none');
        void executeScript(`
          (function() {
            try {
              if (typeof window.__MW_START_REVEAL_WATCH__ === 'function') {
                try {
                  window.__MW_START_REVEAL_WATCH__('fail_closed:${safeReason}', {
                    key: '${safeSovereignId}',
                    force: true,
                    videoId: '${safeVideoId}'
                  });
              ...
              if (typeof window.__MW_APPLY_REVEAL_UI_LOCK__ === 'function') {
                try {
                  window.__MW_APPLY_REVEAL_UI_LOCK__('fail_closed:${safeReason}', ${SHORTS_REVEAL_UI_HARD_FALLBACK_LOCK_MS}, '${safeVideoId}');
                } catch (lockErr) {}
              }
              if (typeof window.__MW_ENSURE_REVEAL_UI__ === 'function') {
                try { window.__MW_ENSURE_REVEAL_UI__('fail_closed:${safeReason}'); } catch (ensureErr) {}
              }
              return true;
            } catch (e) {
              return false;
    ...
    const runPollLoop = async () => {
      ...
      if (stickyShortsMode) {
        if (Date.now() >= shortsProbeDeadlineMs) {
          if (shouldFailOpenShortsProbeTimeout()) {
            deactivateShortsProbe('probe_deadline_elapsed_transport_unknown');
            console.warn(
              '[DIAG][SHORTS_ATOMIC]',
              'action=probe_timeout_fail_open',
              'reason=probe_deadline_elapsed',
              'lastEmpty=' + lastEmptyPollReason,
              'producer=' + lastProducerMode,
        ...
        if (shortsProbePollCount >= SHORTS_LEGACY_FALLBACK_MAX_POLLS) {
          if (shouldFailOpenShortsProbeTimeout()) {
            deactivateShortsProbe('probe_max_polls_transport_unknown');
            console.warn(
              '[DIAG][SHORTS_ATOMIC]',
              'action=probe_max_polls_fail_open',
              'reason=probe_max_polls_reached',
              'lastEmpty=' + lastEmptyPollReason,
              'producer=' + lastProducerMode,
              'status=' + lastPollStatus,
              'navId=' + activeNavIdRef.current,
      ...
      if (stickyShortsMode) {
        ...
        if (document.visibilityState !== 'visible') {
          ...
          if (isLegacyEmpty && lastProducerMode === 'unknown') {
            deactivateShortsProbe('producer_unknown_empty_fail_open');
            console.warn(
              '[DIAG][SHORTS_ATOMIC]',
              'action=producer_unknown_fail_open',
              'reason=producer_unknown_empty',
              'navId=' + activeNavIdRef.current,
            );
            return;
          }
  ...
  const handleRefresh = useCallback(async () => {
    ...
    if (currentView === 'browse' && isNative && webViewState.isOpen) {
      ...
      injectionDoneRef.current = false;
      clearBlurReadySettleWindowState('manual_reload');
      blurReadyRef.current = false;
      clearBlurReadyNavMatchState('manual_reload');
  ...
  }, [readerContent, currentView, searchQuery, isNative, webViewState.isOpen, handleReaderMode, handleSearch, webViewReload, clearBlurReadySettleWindowState, clearBlurReadyNavMatchState, setCentralBlurState, resetShortsOverlayCoordinator, resetOverlayLiftHandshakeState]);
  ...
  const handleHome = useCallback(async () => {
    ...
    injectionDoneRef.current = false;
    clearBlurReadySettleWindowState('home_reset');
    blurReadyRef.current = false;
    clearBlurReadyNavMatchState('home_reset');
    ...
    setFallbackUrl('');
    goHome();
  }, [isNative, webViewState.isOpen, webViewState.currentUrl, closeWebView, goHome, moderationBridge, clearBlurReadySettleWindowState, clearBlurReadyNavMatchState, setCentralBlurState, teardownWebViewScheduling, exitPendingReinject, resetShortsOverlayCoordinator, resetOverlayLiftHandshakeState]);
  ...
  const emergencyRevealContext = (() => {
    if (!isNative || !webViewState.isOpen || currentView !== 'browse') {
      return {
        visible: false,
        sovereignId: 'none',
        videoId: 'none',
    ...
    const sovereignId = getSovereignIdForContext(activeUrl);
    const videoId = parseSovereignId(sovereignId).videoId || 'none';
    if (videoId === 'none') {
      return {
        visible: false,
        sovereignId,
        videoId,
        hostReason: blurStateRef.current.reason || 'none',
      };
    }
    const hasHardRevealLock = !!getShortsHardRevealLockForSovereign(sovereignId);
    if (hasHardRevealLock) {
      return {
        visible: false,
        sovereignId,
        videoId,
    ...
    const pending = shortsOverlayPendingRef.current;
    const pendingForActive = pending && pending.sovereignId === sovereignId ? pending : null;
    const pendingWantsBlur = !!(
      pendingForActive &&
      (
        hasEffectiveShortsBlurSignal(pendingForActive) ||
        pendingForActive.result === 'BLUR' ||
        pendingForActive.mismatchHoldUntil > Date.now()
      )
    );
    const hostWantsBlur = blurStateRef.current.enabled;
    const visible = pendingWantsBlur || hostWantsBlur;
    return {
      visible,
      sovereignId,
      videoId,
  ...
  const handleEmergencyReveal = useCallback(async () => {
    if (!emergencyRevealContext.visible) return;
    const sovereignId = emergencyRevealContext.sovereignId;
    const videoId = emergencyRevealContext.videoId;
    if (!sovereignId || sovereignId === 'none' || !videoId || videoId === 'none') return;
    const reason = 'host_emergency_reveal_button';
    setShortsHardRevealLockForSovereign(sovereignId, reason);
    hydrateShortsPendingStateFromHardRevealLock(sovereignId, reason);
    setCentralBlurState(false, 'host_emergency_reveal');
    queueCurrentBlurState('host_emergency_reveal_button');
    syncShortsRevealRequestToWebViewRef.current(sovereignId, reason);
    await ensureShortsRevealUi(
      'host_emergency_reveal',
      'host_emergency_' + Date.now().toString(36),
      sovereignId,
      videoId,
      0,
    );
    toast.success('Reveal unlocked for this Short');
  }, [
    emergencyRevealContext,
    ensureShortsRevealUi,
    hydrateShortsPendingStateFromHardRevealLock,
    queueCurrentBlurState,
    setCentralBlurState,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {emergencyRevealContext.visible && (
        <div className="fixed bottom-24 left-1/2 z-[85] -translate-x-1/2">
          <button
            onClick={handleEmergencyReveal}
            className="rounded-full border-2 border-white/95 bg-black/75 px-5 py-2 text-sm font-display tracking-wide text-white shadow-[0_10px_28px_rgba(0,0,0,0.45)] backdrop-blur-sm"
            type="button"
          >
            Tap To Reveal
          </button>
