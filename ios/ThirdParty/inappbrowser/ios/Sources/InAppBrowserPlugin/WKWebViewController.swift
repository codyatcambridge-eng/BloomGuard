...
private enum MediaPlaybackAssertionError: Error {
...
open class WKWebViewController: UIViewController, WKScriptMessageHandler {
    ...
    private var mediaPlaybackKeepAliveTask: UIBackgroundTaskIdentifier = .invalid
    ...
    deinit {
        endMediaPlaybackKeepAliveTask(reason: "deinit")
    ...
    private func assertWebKitMediaPlaybackAssertionAvailable(_ webConfiguration: WKWebViewConfiguration) throws {
        let selector = NSSelectorFromString("setAllowsInlineMediaPlayback:")
        guard webConfiguration.responds(to: selector) else {
            throw MediaPlaybackAssertionError.missingEntitlement
        }
    }

    private func beginMediaPlaybackKeepAliveTask(reason: String) {
        if mediaPlaybackKeepAliveTask != .invalid {
            return
        }
        mediaPlaybackKeepAliveTask = UIApplication.shared.beginBackgroundTask(
            withName: "MWWebKitMediaPlaybackKeepAlive"
        ) { [weak self] in
            self?.endMediaPlaybackKeepAliveTask(reason: "expiration")
        }
        if mediaPlaybackKeepAliveTask == .invalid {
#if DEBUG
            print("[InAppBrowser][MediaPlayback] failed to start keep-alive task reason=\(reason)")
#endif
            return
        }
#if DEBUG
        print("[InAppBrowser][MediaPlayback] keep-alive fallback active reason=\(reason)")
#endif
    }

    private func endMediaPlaybackKeepAliveTask(reason: String) {
        guard mediaPlaybackKeepAliveTask != .invalid else {
            return
        }
        UIApplication.shared.endBackgroundTask(mediaPlaybackKeepAliveTask)
        mediaPlaybackKeepAliveTask = .invalid
#if DEBUG
        print("[InAppBrowser][MediaPlayback] keep-alive fallback ended reason=\(reason)")
#endif
    }

    private func configureMediaPlaybackAssertionSafely(_ webConfiguration: WKWebViewConfiguration) {
        do {
            try assertWebKitMediaPlaybackAssertionAvailable(webConfiguration)
            webConfiguration.allowsInlineMediaPlayback = true
            endMediaPlaybackKeepAliveTask(reason: "assertion_available")
        } catch {
            webConfiguration.allowsInlineMediaPlayback = true
            beginMediaPlaybackKeepAliveTask(reason: "missing_entitlement")
#if DEBUG
            print("[InAppBrowser][MediaPlayback] assertion unavailable, using keep-alive fallback error=\(error)")
#endif
        }
    }
    ...
    open func initWebview(isInspectable: Bool = true) {
        ...
        configureMediaPlaybackAssertionSafely(webConfiguration)
        ...
        let topConstraint = webView.topAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.topAnchor)
        ...
        let bottomConstraint = webView.bottomAnchor.constraint(equalTo: bottomPadding)
        [topConstraint, leadingConstraint, trailingConstraint, bottomConstraint].forEach {
...
public extension WKWebViewController {
    ...
    open func cleanupWebView() {
        endMediaPlaybackKeepAliveTask(reason: "cleanup")
...
class PassThroughView: UIView {
    ...
    override func layoutSubviews() {
        ...
        // Some hosted presentation paths inject a zero-width encapsulated constraint.
        // When that happens, force this container back to full screen width.
        let encapsulatedWidthConstraint = constraints.first {
            $0.firstAttribute == .width &&
                (
                    $0.identifier == "UIView-Encapsulated-Layout-Width" ||
                        (($0.firstItem as AnyObject?) === self && $0.secondItem == nil)
                )
        }

        let hasZeroWidth = bounds.width <= 0.5 || frame.width <= 0.5
        let hasZeroEncapsulatedWidth = (encapsulatedWidthConstraint?.constant ?? 1) <= 0.5
        if !hasZeroWidth && !hasZeroEncapsulatedWidth { return }

        if let widthConstraint = encapsulatedWidthConstraint, widthConstraint.constant <= 0.5 {
            widthConstraint.constant = fallbackWidth
        }

        if frame.width <= 0.5 {
            var repairedFrame = frame
            repairedFrame.size.width = fallbackWidth
            frame = repairedFrame
        }
        if bounds.width <= 0.5 {
            var repairedBounds = bounds
            repairedBounds.size.width = fallbackWidth
